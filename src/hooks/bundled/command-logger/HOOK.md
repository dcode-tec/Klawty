...
homepage: https://docs.klawty.ai/automation/hooks#command-logger
metadata:
  {
    "klawty":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.klawty/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.klawty/logs/commands.log ~/.klawty/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/klawty`:
   ```
   /home/username/.klawty/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
