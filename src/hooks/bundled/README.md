...
This directory contains hooks that ship with Klawty. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.klawty/klawty.json`:
...
2. Restart gateway: `pkill -9 -f 'klawty.*gateway' && pnpm klawty gateway`
3. Enable the hook: `klawty hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.klawty.ai/automation/hooks
