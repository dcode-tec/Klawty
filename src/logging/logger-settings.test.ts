...
let originalKlawtyLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.KLAWTY_TEST_FILE_LOG;
  originalKlawtyLogLevel = process.env.KLAWTY_LOG_LEVEL;
  delete process.env.KLAWTY_TEST_FILE_LOG;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.KLAWTY_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalKlawtyLogLevel === undefined) {
    delete process.env.KLAWTY_LOG_LEVEL;
  } else {
    process.env.KLAWTY_LOG_LEVEL = originalKlawtyLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
