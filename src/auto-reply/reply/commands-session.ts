...
import { scheduleGatewaySigusr1Restart, triggerKlawtyRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Klawty in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerKlawtyRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Klawty via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
