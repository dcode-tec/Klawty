...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop klawty!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@klawty_bot", { botUsername: "klawty_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@klawty_bot", { botUsername: "klawty_bot" })).toBe(true);
