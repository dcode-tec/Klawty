...
const workspaceHarness = createCommandWorkspaceHarness("klawty-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".klawty", "extensions", params.pluginId);
...
function buildCfg(): KlawtyConfig {
  return {
    commands: {
      text: true,
      plugins: true,
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("klawty-command-plugins-home-", async () => {
