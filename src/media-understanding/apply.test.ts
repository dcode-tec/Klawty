...
const TEMP_MEDIA_PREFIX = "klawty-media-";
...
function createGroqAudioConfig(): KlawtyConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): KlawtyConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): KlawtyConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: KlawtyConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredKlawtyTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        KLAWTY_AGENT_DIR: isolatedAgentDir,
