...
function listAgents(cfg: KlawtyConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: KlawtyConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<KlawtyConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: KlawtyConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: KlawtyConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: KlawtyConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<KlawtyConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  KlawtyConfig,
  {
    bindingsRef: KlawtyConfig["bindings"];
    agentsRef: KlawtyConfig["agents"];
    sessionRef: KlawtyConfig["session"];
...
function resolveRouteCacheForConfig(cfg: KlawtyConfig): Map<string, ResolvedAgentRoute> {
