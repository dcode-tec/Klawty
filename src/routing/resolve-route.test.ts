...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: KlawtyConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: KlawtyConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/klawty-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<KlawtyConfig["bindings"]>[number];
