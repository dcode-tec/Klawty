...
const { makeStorePath } = createCronStoreHarness({ prefix: "klawty-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses KLAWTY_HOME for tilde expansion", () => {
    vi.stubEnv("KLAWTY_HOME", "/srv/klawty-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/klawty-home", "cron", "jobs.json"));
  });
});
