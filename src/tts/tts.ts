...
export function resolveTtsConfig(cfg: KlawtyConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.KLAWTY_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: KlawtyConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: KlawtyConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      const tempRoot = resolvePreferredKlawtyTmpDir();
