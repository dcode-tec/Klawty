...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set KLAWTY_GATEWAY_TOKEN/KLAWTY_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.KLAWTY_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.KLAWTY_GATEWAY_PASSWORD);
