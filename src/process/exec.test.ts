...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        KLAWTY_BASE_ENV: "base",
        KLAWTY_TO_REMOVE: undefined,
      },
      env: {
        KLAWTY_TEST_ENV: "ok",
      },
    ...
    expect(resolved.KLAWTY_BASE_ENV).toBe("base");
    expect(resolved.KLAWTY_TEST_ENV).toBe("ok");
    expect(resolved.KLAWTY_TO_REMOVE).toBeUndefined();
    expect(resolved.KLAWTY_CLI).toBe(KLAWTY_CLI_ENV_VALUE);
  });
