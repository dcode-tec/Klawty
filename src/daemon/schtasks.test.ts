...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when KLAWTY_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".klawty", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when KLAWTY_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", KLAWTY_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".klawty-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers KLAWTY_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        KLAWTY_PROFILE: "rescue",
        KLAWTY_STATE_DIR: "C:\\State\\klawty",
      },
      expected: path.join("C:\\State\\klawty", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", KLAWTY_PROFILE: "default" },
      expected: path.join("/home/test", ".klawty", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Klawty Gateway",
          "cd /d C:\\Projects\\klawty",
          "set NODE_ENV=production",
          "set KLAWTY_PORT=2508",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\klawty",
          environment: {
            NODE_ENV: "production",
            KLAWTY_PORT: "2508",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\klawty\\dist\\index.js gateway --port 2508',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\klawty\\dist\\index.js",
            "gateway",
            "--port",
            "2508",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Klawty Share\\node.exe" "\\\\fileserver\\Klawty Share\\dist\\index.js" gateway --port 2508',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Klawty Share\\node.exe",
            "\\\\fileserver\\Klawty Share\\dist\\index.js",
            "gateway",
            "--port",
            "2508",
          ],
  ...
  it("reads script from KLAWTY_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ KLAWTY_STATE_DIR: path.join(tmpDir, "custom-state") }),
