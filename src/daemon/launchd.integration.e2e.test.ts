...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `klawty-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      KLAWTY_LAUNCHD_LABEL: `ai.klawty.launchd-int-${testId}`,
      KLAWTY_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
