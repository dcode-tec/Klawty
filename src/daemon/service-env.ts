...
export function buildServiceEnvironment(params: {
  ...
  return {
    ...
    KLAWTY_PROFILE: profile,
    KLAWTY_GATEWAY_PORT: String(port),
    KLAWTY_LAUNCHD_LABEL: resolvedLaunchdLabel,
    KLAWTY_SYSTEMD_UNIT: systemdUnit,
    KLAWTY_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    KLAWTY_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    KLAWTY_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform, extraPathDirs);
  const gatewayToken =
    env.KLAWTY_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    KLAWTY_GATEWAY_TOKEN: gatewayToken,
    KLAWTY_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    KLAWTY_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    KLAWTY_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    KLAWTY_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    KLAWTY_LOG_PREFIX: "node",
    KLAWTY_SERVICE_MARKER: NODE_SERVICE_MARKER,
    KLAWTY_SERVICE_KIND: NODE_SERVICE_KIND,
    KLAWTY_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    KLAWTY_STATE_DIR: sharedEnv.stateDir,
    KLAWTY_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.KLAWTY_STATE_DIR;
  const configPath = env.KLAWTY_CONFIG_PATH;
