...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds KLAWTY_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `klawty gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service KLAWTY_GATEWAY_TOKEN does not match gateway.auth.token in klawty.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.KLAWTY_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.KLAWTY_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `klawty gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
