...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "KLAWTY_BUNDLED_PLUGINS_DIR",
  "KLAWTY_SKIP_CHANNELS",
  "KLAWTY_SKIP_PROVIDERS",
  "KLAWTY_SKIP_CRON",
  "KLAWTY_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.KLAWTY_SKIP_PROVIDERS = "1";
  process.env.KLAWTY_SKIP_CRON = "1";
  process.env.KLAWTY_TEST_MINIMAL_GATEWAY = "1";
  process.env.KLAWTY_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "klawty-test-no-bundled-extensions")
    : "klawty-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".klawty-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".klawty-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__klawtyTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__klawtyTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["KLAWTY_GATEWAY_TOKEN"]);
  const prev = process.env.KLAWTY_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.KLAWTY_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.KLAWTY_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
