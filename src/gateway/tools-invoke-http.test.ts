...
let lastCreateKlawtyToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/klawty-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateKlawtyToolsContext?.agentTo,
          agentThreadId: lastCreateKlawtyToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createKlawtyTools: (ctx: Record<string, unknown>) => {
      lastCreateKlawtyToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateKlawtyToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateKlawtyToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateKlawtyToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-klawty-message-to": "channel:24514",
        "x-klawty-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
