...
const { createKlawtyTools } = await import("../agents/klawty-tools.js");
...
type SessionSendTool = ReturnType<typeof createKlawtyTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createKlawtyTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["KLAWTY_GATEWAY_PORT", "KLAWTY_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.KLAWTY_GATEWAY_PORT = String(gatewayPort);
  process.env.KLAWTY_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("KLAWTY_CONFIG_PATH missing in gateway test environment");
      }
