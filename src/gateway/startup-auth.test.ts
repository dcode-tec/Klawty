...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: KlawtyConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: KlawtyConfig) {
  ...
  async function expectNoTokenGeneration(cfg: KlawtyConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): KlawtyConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
    ...
    await expectResolvedToken({
      cfg: {
        gateway: {
          auth: {
            mode: "token",
            token: "${KLAWTY_GATEWAY_TOKEN}",
          },
        },
      },
      env: {
        KLAWTY_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${KLAWTY_GATEWAY_TOKEN}",
    });
  });

  it("uses KLAWTY_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        KLAWTY_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
