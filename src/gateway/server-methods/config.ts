...
import type { ConfigValidationIssue, KlawtyConfig } from "../../config/types.klawty.js";
import {
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: KlawtyConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadKlawtyPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    runtimeOptions: {
  ...
  // our cache. However, both loadConfig() and loadKlawtyPlugins() already cache
