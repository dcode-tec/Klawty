...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("klawty-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "klawty-sandbox-browser-global" },
      agentBrowser: { network: "klawty-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("klawty-sandbox-browser-agent");
  });
