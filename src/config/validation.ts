...
import type { KlawtyConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: KlawtyConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: KlawtyConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: KlawtyConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = KlawtySchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as KlawtyConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as KlawtyConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as KlawtyConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as KlawtyConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; Klawty mapped it automatically for compatibility.",
  }));
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Klawty config schema.
