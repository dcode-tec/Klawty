...
describe("legacy migrate controlUi.allowedOrigins seed (issue #29385)", () => {
  ...
  it("seeds allowedOrigins including custom bind host for bind=custom", () => {
    ...
    expect(res.config?.gateway?.controlUi?.allowedOrigins).toContain("http://192.168.1.100:2508");
    expect(res.config?.gateway?.controlUi?.allowedOrigins).toContain("http://localhost:2508");
  });
