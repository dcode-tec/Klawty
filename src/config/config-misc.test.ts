...
import { buildWebSearchProviderConfig, withTempHome, writeKlawtyConfig } from "./test-helpers.js";
import { KlawtySchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = KlawtySchema.safeParse({
      $schema: "https://klawty.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://klawty.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = KlawtySchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = KlawtySchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeKlawtyConfig(home, {
        gateway: { bind: "${KLAWTY_BIND}" },
      });

      const prev = process.env.KLAWTY_BIND;
      process.env.KLAWTY_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.KLAWTY_BIND;
        } else {
          process.env.KLAWTY_BIND = prev;
        }
      }
    });
  });
