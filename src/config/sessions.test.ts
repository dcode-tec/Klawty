...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ KLAWTY_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-klawty",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-klawty#general");
  });
  ...
  it("derives session transcripts dir from KLAWTY_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { KLAWTY_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when KLAWTY_STATE_DIR differs from stored paths", () => {
