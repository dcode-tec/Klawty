...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".klawty");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.KLAWTY_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$KLAWTY_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via KLAWTY_STATE_DIR.
