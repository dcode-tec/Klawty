...
describe("matrix migration snapshots", () => {
  ...
  it("creates a backup marker after writing a pre-migration snapshot", async () => {
    await withTempHome(async (home) => {
      const archivePath = path.join(home, "Backups", "klawty-migrations", "snapshot.tar.gz");
      fs.mkdirSync(path.dirname(archivePath), { recursive: true });
      fs.writeFileSync(path.join(home, ".klawty", "klawty.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(home, ".klawty", "state.txt"), "state\n", "utf8");
    ...
    await withTempHome(async (home) => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          version: 1,
          createdAt: "2026-03-10T18:00:00.000Z",
          archivePath: path.join(home, "Backups", "klawty-migrations", "missing.tar.gz"),
          trigger: "older-run",
          includeWorkspace: false,
        }),
        "utf8",
