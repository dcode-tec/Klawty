...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers KLAWTY_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for KLAWTY_HOME", () => {
    const result = resolveRequiredHomeDir(
      { KLAWTY_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when KLAWTY_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ KLAWTY_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { KLAWTY_HOME: "/srv/klawty-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/klawty-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/klawty-home " },
      expected: "/srv/klawty-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/klawty-home/docs"));
  });
