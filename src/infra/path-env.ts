...
function candidateBinDirs(opts: EnsureKlawtyPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `klawty` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "klawty");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.KLAWTY_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "klawty"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `klawty` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureKlawtyCliOnPath(opts: EnsureKlawtyPathOpts = {}) {
  if (isTruthyEnvValue(process.env.KLAWTY_PATH_BOOTSTRAPPED)) {
    return;
  }
