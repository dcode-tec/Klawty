 ...
 * returning control to the caller (typically `triggerKlawtyRestart` →
...
/**
 * Parse klawty gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("klawty")) {
 ...
 * Returns only PIDs that belong to klawty gateway processes (not the current process).
 */
