...
const mockResolveGatewayPort = vi.hoisted(() => vi.fn(() => 2508));
...
function createKlawtyBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "klawty-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createKlawtyBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  beforeEach(async () => {
    ...
    mockResolveGatewayPort.mockReturnValue(2508);
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses klawty-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "klawty-gateway" },
          { pid: process.pid, cmd: "klawty-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'klawty'", () => {
    ...
    it("forwards the spawnTimeoutMs argument to spawnSync", () => {
      ...
      findGatewayPidsOnPortSync(2508, 400);
    ...
    it("deduplicates pids from dual-stack listeners (IPv4+IPv6 emit same pid twice)", () => {
      ...
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(2508);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple klawty pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "klawty-gateway" },
          { pid: pid2, cmd: "klawty-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(2508);
    ...
    it("returns [] when status 0 but only non-klawty pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks klawty processes — non-klawty listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty klawty stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty klawty stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + klawty pid in stdout — container-restricted lsof reports partial results
          return createKlawtyBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "klawty-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'klawty'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("klawty")
      // inside the mid-loop flush: a non-klawty cmd between two entries must not
      // be pushed, but the following klawty entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-klawty entry first, then klawty entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid klawty entry must still be found.
      const stdout = `p0\ncklawty-gateway\np${stalePid}\ncklawty-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(2508);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero klawty pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-klawty stdout (line 145)", () => {
    it("treats status 1 + non-klawty stdout as port-free (not an klawty process)", () => {
      // status 1 + non-empty stdout where no klawty pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill klawty pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-klawty output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no klawty pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
