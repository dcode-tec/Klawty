...
import { resolveKlawtyPackageRoot, resolveKlawtyPackageRootSync } from "./klawty-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveKlawtyPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "klawty" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveKlawtyPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/klawty -> .../dist/index.js)
