...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevKlawtyHome = process.env.KLAWTY_HOME;
    const prevHome = process.env.HOME;
    process.env.KLAWTY_HOME = "/srv/klawty-home";
    process.env.HOME = "/home/other";
    const klawtyHome = path.join(path.resolve("/srv/klawty-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", klawtyHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevKlawtyHome === undefined) {
        delete process.env.KLAWTY_HOME;
      } else {
        process.env.KLAWTY_HOME = prevKlawtyHome;
      }
