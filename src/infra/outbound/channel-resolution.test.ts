...
const loadKlawtyPluginsMock = vi.hoisted(() => vi.fn());
...
vi.mock("../../plugins/loader.js", () => ({
  loadKlawtyPlugins: (...args: unknown[]) => loadKlawtyPluginsMock(...args),
}));
...
describe("outbound channel resolution", () => {
  beforeEach(() => {
    ...
    loadKlawtyPluginsMock.mockReset();
  ...
  it("returns the already-registered plugin without bootstrapping", async () => {
    ...
    expect(loadKlawtyPluginsMock).not.toHaveBeenCalled();
  });
  ...
  it("bootstraps plugins once per registry key and returns the newly loaded plugin", async () => {
    ...
    expect(loadKlawtyPluginsMock).toHaveBeenCalledWith({
    ...
    expect(loadKlawtyPluginsMock).toHaveBeenCalledTimes(1);
    expect(loadKlawtyPluginsMock).toHaveBeenLastCalledWith({
      config: { autoEnabled: true },
      workspaceDir: "/tmp/workspace",
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
  ...
  it("retries bootstrap after a transient load failure", async () => {
    getChannelPluginMock.mockReturnValue(undefined);
    loadKlawtyPluginsMock.mockImplementationOnce(() => {
    ...
    expect(loadKlawtyPluginsMock).toHaveBeenCalledTimes(2);
  });
});
