...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: KlawtyConfig | null = null;
  ...
  get(key: string, cfg: KlawtyConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: KlawtyConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: KlawtyConfig): void {
