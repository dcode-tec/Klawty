...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Klawty tmp root", async () => {
      const tmpRoot = resolvePreferredKlawtyTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideKlawty = path.join(os.tmpdir(), "outside-klawty", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
