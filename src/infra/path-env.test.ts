...
let ensureKlawtyCliOnPath: typeof import("./path-env.js").ensureKlawtyCliOnPath;

describe("ensureKlawtyCliOnPath", () => {
  const envKeys = [
    "PATH",
    "KLAWTY_PATH_BOOTSTRAPPED",
    "KLAWTY_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureKlawtyCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/klawty-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "klawty");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureKlawtyCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling klawty exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.KLAWTY_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.KLAWTY_PATH_BOOTSTRAPPED = "1";
    ensureKlawtyCliOnPath({
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "klawty");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.KLAWTY_PATH_BOOTSTRAPPED;
      delete process.env.KLAWTY_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.KLAWTY_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/klawty-path/case-linuxbrew");
