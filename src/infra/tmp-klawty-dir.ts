...
export const POSIX_KLAWTY_TMP_DIR = "/tmp/klawty";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredKlawtyTmpDirOptions = {
...
export function resolvePreferredKlawtyTmpDir(
  options: ResolvePreferredKlawtyTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "klawty" : `klawty-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[klawty] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Klawty temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Klawty temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_KLAWTY_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_KLAWTY_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_KLAWTY_TMP_DIR)) {
      return POSIX_KLAWTY_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_KLAWTY_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_KLAWTY_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_KLAWTY_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_KLAWTY_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_KLAWTY_TMP_DIR;
