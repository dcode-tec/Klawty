...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "klawty.internal", expected: "klawty.internal." },
    { value: "klawty.internal.", expected: "klawty.internal." },
    { value: "  klawty.internal  ", expected: "klawty.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { KLAWTY_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { KLAWTY_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("klawty.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "klawty.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expect(txt).toContain(`$ORIGIN klawty.internal.`);
    ...
    expect(txt).toContain(`_klawty-gw._tcp IN PTR studio-london._klawty-gw._tcp`);
    expect(txt).toContain(`studio-london._klawty-gw._tcp IN SRV 0 0 2508 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Klawty)`);
    expect(txt).toContain(`gatewayPort=2508`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/klawty`);
  });
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN klawty.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._klawty-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (Klawty)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("klawty.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("klawty.internal."),
