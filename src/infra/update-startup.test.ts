...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["KLAWTY_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.KLAWTY_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveKlawtyPackageRoot } = await import("./klawty-root.js"));
    ...
    vi.mocked(resolveKlawtyPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveKlawtyPackageRoot).mockResolvedValue("/opt/klawty");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/klawty/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
