...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __klawtyDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__klawtyDiagnosticEventsState) {
    globalStore.__klawtyDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__klawtyDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: KlawtyConfig): boolean {
