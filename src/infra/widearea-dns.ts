...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.KLAWTY_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*klawty-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "klawty";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "klawty");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "klawty-gw");
  ...
  records.push(`_klawty-gw._tcp IN PTR ${instanceLabel}._klawty-gw._tcp`);
  records.push(`${instanceLabel}._klawty-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._klawty-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; klawty-content-hash: ${contentHash}\n${contentBody}`;
}
