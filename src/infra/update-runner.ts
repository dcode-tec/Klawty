...
const DEFAULT_PACKAGE_NAME = "klawty";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["klawty", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "klawty.mjs");
    ...
    const doctorStep = await runStep(
      step("klawty doctor", doctorArgv, gitRoot, { KLAWTY_UPDATE_IN_PROGRESS: "1" }),
    );
