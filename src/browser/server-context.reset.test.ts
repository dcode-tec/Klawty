...
function localKlawtyProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "klawty",
...
function createLocalKlawtyResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localKlawtyProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveKlawtyUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "klawty-reset-"));
    const profileDir = path.join(tempRoot, "klawty");
    ...
    const ops = createLocalKlawtyResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveKlawtyUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "klawty-reset-no-own-"));
    const profileDir = path.join(tempRoot, "klawty");
    fs.mkdirSync(profileDir, { recursive: true });
