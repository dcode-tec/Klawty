  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          klawty: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.KLAWTY_GATEWAY_PORT;
    process.env.KLAWTY_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.KLAWTY_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.KLAWTY_GATEWAY_PASSWORD;
