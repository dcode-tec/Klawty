...
vi.mock("./chrome.js", () => ({
  resolveKlawtyUserDataDir: vi.fn(() => "/tmp/klawty-test/klawty/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "klawty-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveKlawtyUserDataDir).mockReturnValue(userDataDir);
