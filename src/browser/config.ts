import type { BrowserConfig, BrowserProfileConfig, KlawtyConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_KLAWTY_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_KLAWTY_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "klawty" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_KLAWTY_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_KLAWTY_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: KlawtyConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_KLAWTY_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_KLAWTY_BROWSER_PROFILE_NAME]
        ? DEFAULT_KLAWTY_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "klawty";
