...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "klawty" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.KLAWTY_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "klawty",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/klawty" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    klawty: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchKlawtyChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveKlawtyUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopKlawtyChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "klawty";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.KLAWTY_GATEWAY_PORT;
  process.env.KLAWTY_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.KLAWTY_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.KLAWTY_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.KLAWTY_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.KLAWTY_GATEWAY_PASSWORD;
  } else {
    process.env.KLAWTY_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
