...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredKlawtyTmpDir: vi.fn(() => "/tmp/klawty"),
}));
vi.mock("../infra/tmp-klawty-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredKlawtyTmpDir.mockReturnValue("/tmp/klawty");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".klawty-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredKlawtyTmpDir.mockReturnValue("/tmp/klawty-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "klawty-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "klawty-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredKlawtyTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "klawty-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "klawty-preferred", "downloads")}${path.sep}`),
    );
  });
