...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(2508)).toEqual([]);
  });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(2508)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(2508);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(2508, {
      timeoutMs: 500,
      intervalMs: 100,
  ...
  it("falls back to fuser when lsof is permission denied", async () => {
    (execFileSync as unknown as Mock).mockImplementation((cmd: string) => {
      ...
      return "2508/tcp: 4242\n";
    });
    tryListenOnPortMock.mockResolvedValue(undefined);

    const result = await forceFreePortAndWait(2508, { timeoutMs: 500, intervalMs: 100 });
  ...
  it("throws when lsof is unavailable and fuser is missing", async () => {
    ...
    await expect(forceFreePortAndWait(2508, { timeoutMs: 200, intervalMs: 100 })).rejects.toThrow(
      /fuser not found/i,
    );
  });
});
...
describe("gateway --force helpers (Windows netstat path)", () => {
  ...
  it("parses PIDs from netstat output correctly", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(2508, 42, 99));
    expect(listPortListeners(2508)).toEqual<PortProcess[]>([{ pid: 42 }, { pid: 99 }]);
  });
  ...
  it("deduplicates PIDs that appear multiple times", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(2508, 42, 42));
    expect(listPortListeners(2508)).toEqual<PortProcess[]>([{ pid: 42 }]);
  });
  ...
  it("throws a descriptive error when netstat fails", () => {
    ...
    expect(() => listPortListeners(2508)).toThrow(/netstat failed/);
  });
