...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["klawty update", "Update a source checkout (git)"],
        ["klawty update --channel beta", "Switch to beta channel (git + npm)"],
        ["klawty update --channel dev", "Switch to dev channel (git + npm)"],
        ["klawty update --tag beta", "One-off update to a dist-tag or version"],
        ["klawty update --tag main", "One-off package install from GitHub main"],
        ["klawty update --dry-run", "Preview actions without changing anything"],
        ["klawty update --no-restart", "Update without restarting the service"],
        ["klawty update --json", "Output result as JSON"],
        ["klawty update --yes", "Non-interactive (accept downgrade prompts)"],
        ["klawty update wizard", "Interactive update wizard"],
        ["klawty --update", "Shorthand for klawty update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run klawty update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.klawty.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["klawty update status", "Show channel + version status."],
          ["klawty update status --json", "JSON output."],
          ["klawty update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.klawty.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
