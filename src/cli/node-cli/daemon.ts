...
function renderNodeServiceStartHints(): string[] {
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("klawty node install"),
    startCommand: formatCliCommand("klawty node start"),
...
function resolveNodeDefaults(
  ...
  const port = portOverride ?? config?.gateway?.port ?? 2508;
  return { host, port };
}
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("klawty node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    KLAWTY_LOG_PREFIX: baseEnv.KLAWTY_LOG_PREFIX ?? "node",
