...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["klawty security audit", "Run a local security audit."],
          ["klawty security audit --deep", "Include best-effort live Gateway probe checks."],
          ["klawty security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "klawty security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["klawty security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["klawty security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.klawty.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Klawty security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("klawty security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("klawty security audit --fix")}`));
        if (!fixResult) {
