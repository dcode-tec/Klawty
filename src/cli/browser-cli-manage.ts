...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "klawty";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "klawty" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (klawty|existing-session). Default: klawty")
    .action(
      async (
        opts: {
          name: string;
