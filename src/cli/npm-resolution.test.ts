...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@klawty/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@klawty/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@klawty/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@klawty/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @klawty/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@klawty/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@klawty/plugin-alpha@1.2.3",
        installPath: "/tmp/klawty/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@klawty/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @klawty/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@klawty/plugin-alpha@latest",
      true,
      "/tmp/klawty/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@klawty/plugin-alpha@latest",
      installPath: "/tmp/klawty/extensions/alpha",
      version: "1.2.3",
