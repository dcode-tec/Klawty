...
beforeEach(() => {
  ...
  originalHideBanner = process.env.KLAWTY_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.KLAWTY_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.KLAWTY_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("klawty");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "klawty", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("klawty-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "klawty", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "klawty", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "klawty", "channels", "add"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "klawty", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.KLAWTY_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "klawty", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "klawty", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "klawty", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "klawty", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "klawty", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "klawty", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "klawty", "backup", "create", "--json"],
    });
