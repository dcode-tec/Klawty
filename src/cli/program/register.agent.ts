...
${formatHelpExamples([
  ['klawty agent --to +15555550123 --message "status update"', "Start a new session."],
  ['klawty agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'klawty agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'klawty agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['klawty agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'klawty agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.klawty.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.klawty.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['klawty agents set-identity --agent main --name "Klawty" --emoji "🦞"', "Set name + emoji."],
  ["klawty agents set-identity --agent main --avatar avatars/klawty.png", "Set avatar path."],
  [
    "klawty agents set-identity --workspace ~/.klawty/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "klawty agents set-identity --identity-file ~/.klawty/workspace/IDENTITY.md --agent main",
