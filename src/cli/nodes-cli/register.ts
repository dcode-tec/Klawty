    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["klawty nodes status", "List known nodes with live status."],
          ["klawty nodes pairing pending", "Show pending node pairing requests."],
          ['klawty nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["klawty nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.klawty.ai/cli/nodes")}\n`,
    );
