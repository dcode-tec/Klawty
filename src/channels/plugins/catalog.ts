...
import { discoverKlawtyPlugins } from "../../plugins/discovery.js";
import { loadPluginManifest } from "../../plugins/manifest.js";
import type { KlawtyPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, KlawtyPackageManifest>>;

const ENV_CATALOG_PATHS = ["KLAWTY_PLUGIN_CATALOG_PATHS", "KLAWTY_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<KlawtyPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: KlawtyPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  origin?: PluginOrigin;
  workspaceDir?: string;
  packageManifest?: KlawtyPackageManifest;
...
function loadBundledMetadataCatalogEntries(options: CatalogOptions): ChannelPluginCatalogEntry[] {
  ...
  for (const dirent of fs.readdirSync(bundledDir, { withFileTypes: true })) {
    ...
    const entry = buildCatalogEntry({
      ...
      packageManifest: packageJson.klawty,
    });
    if (entry) {
      entries.push(entry);
    }
