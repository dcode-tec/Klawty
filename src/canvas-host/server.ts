...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.klawtyCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.klawtyCanvasA2UIAction &&
        typeof window.klawtyCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.klawtySendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("klawty:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Klawty node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.klawtySendUserAction === "function"
        ? window.klawtySendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.KLAWTY_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.KLAWTY_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Klawty Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
