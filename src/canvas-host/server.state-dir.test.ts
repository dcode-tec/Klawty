...
describe("canvas host state dir defaults", () => {
  it("uses KLAWTY_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("klawty-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Klawty Canvas");
      } finally {
        await handler.close();
      }
    });
