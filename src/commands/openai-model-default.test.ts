...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: KlawtyConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: KlawtyConfig) => { changed: boolean; next: KlawtyConfig };
  defaultModel: string;
  overrideConfig: KlawtyConfig;
  alreadyDefaultConfig: KlawtyConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: KlawtyConfig) => config,
      applyDefaultConfig: (config: KlawtyConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: KlawtyConfig = { agents: { defaults: {} } };
