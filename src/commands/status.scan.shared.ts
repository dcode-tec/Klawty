...
export function hasExplicitMemorySearchConfig(cfg: KlawtyConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: KlawtyConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: KlawtyConfig, agentId: string) => { store: { path: string } } | null;
