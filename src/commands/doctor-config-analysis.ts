...
import { KlawtySchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: KlawtyConfig): {
  config: KlawtyConfig;
  removed: string[];
} {
  const parsed = KlawtySchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: KlawtyConfig): void {
