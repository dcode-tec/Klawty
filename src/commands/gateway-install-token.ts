...
import { readConfigFileSnapshot, writeConfigFile, type KlawtyConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("klawty config set gateway.auth.mode token")} or ${formatCliCommand("klawty config set gateway.auth.mode password")}.`,
  ].join(" ");
}
