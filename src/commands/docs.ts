...
const SEARCH_TOOL = "https://docs.klawty.ai/mcp.SearchKlawty";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.klawty.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('klawty docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.klawty.ai/");
      runtime.log(`Search: ${formatCliCommand('klawty docs "your query"')}`);
    }
    return;
  }
