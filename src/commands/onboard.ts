...
export async function setupWizardCommand(
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        "Read: https://docs.klawty.ai/security",
        `Re-run with: ${formatCliCommand("klawty onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected - Klawty runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.klawty.ai/windows",
      ].join("\n"),
    );
  }
