  ...
  it("defaults discovered direct remote URLs to wss://", async () => {
    ...
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.initialValue).toBe("wss://gateway.tailnet.ts.net:2508");
    ...
    expect(next.gateway?.remote?.url).toBe("wss://gateway.tailnet.ts.net:2508");
  ...
  it("validates insecure ws:// remote URLs and allows only loopback ws:// by default", async () => {
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        ...
        expect(params.validate?.("ws://203.0.113.10:2508")).toContain("Use wss://");
        // ws:// to private IPs remains blocked by default
        expect(params.validate?.("ws://10.0.0.8:2508")).toContain("Use wss://");
        expect(params.validate?.("ws://127.0.0.1:2508")).toBeUndefined();
        expect(params.validate?.("wss://remote.example.com:2508")).toBeUndefined();
        return "wss://remote.example.com:2508";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("wss://remote.example.com:2508");
    expect(next.gateway?.remote?.token).toBeUndefined();
  });

  it("allows ws:// hostname remote URLs when KLAWTY_ALLOW_INSECURE_PRIVATE_WS=1", async () => {
    process.env.KLAWTY_ALLOW_INSECURE_PRIVATE_WS = "1";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        expect(params.validate?.("ws://klawty-gateway.ai:2508")).toBeUndefined();
        expect(params.validate?.("ws://1.1.1.1:2508")).toContain("Use wss://");
        return "ws://klawty-gateway.ai:2508";
      }
      return "";
    }) as WizardPrompter["text"];
    ...
    expect(next.gateway?.mode).toBe("remote");
    expect(next.gateway?.remote?.url).toBe("ws://klawty-gateway.ai:2508");
  });

  it("supports storing remote auth as an external env secret ref", async () => {
    process.env.KLAWTY_GATEWAY_TOKEN = "remote-token-value";
    const text: WizardPrompter["text"] = vi.fn(async (params) => {
      if (params.message === "Gateway WebSocket URL") {
        return "wss://remote.example.com:2508";
      }
      if (params.message === "Environment variable name") {
        return "KLAWTY_GATEWAY_TOKEN";
      }
      return "";
    }) as WizardPrompter["text"];
