...
import { resolveKlawtyPackageRoot } from "../infra/klawty-root.js";
...
function resolveMode(cfg: KlawtyConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveKlawtyPackageRoot({
  ...
  let cfg: KlawtyConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("klawty configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("klawty config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("klawty setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("klawty config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("klawty config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("klawty doctor --fix")}" to apply changes.`);
  }
