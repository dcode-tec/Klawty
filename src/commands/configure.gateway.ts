...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "KLAWTY_GATEWAY_TOKEN",
          placeholder: "KLAWTY_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like KLAWTY_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Klawty will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Klawty trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.klawty.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
