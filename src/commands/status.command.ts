...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByKlawty ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByKlawty ? "installed · " : "";
  ...
  runtime.log(theme.heading("Klawty status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`klawty devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("klawty devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("klawty devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("klawty security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("klawty security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.klawty.ai/faq");
  runtime.log("Troubleshooting: https://docs.klawty.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("klawty status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("klawty logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("klawty status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("klawty gateway probe")}`);
  }
}
