...
const ensureKlawtyModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveKlawtyAgentDir = vi.fn().mockReturnValue("/tmp/klawty-agent");
const ensureAuthProfileStore = vi.fn().mockReturnValue({ version: 1, profiles: {} });
const listProfilesForProvider = vi.fn().mockReturnValue([]);
const resolveAuthProfileDisplayLabel = vi.fn(({ profileId }: { profileId: string }) => profileId);
const resolveAuthStorePathForDisplay = vi
  .fn()
  .mockReturnValue("/tmp/klawty-agent/auth-profiles.json");
...
vi.mock("../agents/models-config.js", () => ({
  ensureKlawtyModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveKlawtyAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureKlawtyModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureKlawtyModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureKlawtyModelsJson).toHaveBeenCalled();
    expect(ensureKlawtyModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
