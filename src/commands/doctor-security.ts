...
import type { KlawtyConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: KlawtyConfig): string[] {
...
export async function noteSecurityWarnings(cfg: KlawtyConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("klawty security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.klawty/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("klawty approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Example tunnel: ssh -N -L 2508:127.0.0.1:2508 user@gateway-host",
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("klawty configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("klawty config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("klawty doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("klawty config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('klawty config set session.dmScope "per-channel-peer"') +
