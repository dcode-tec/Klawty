...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/klawty-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/klawty-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "klawty-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<KlawtyConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<KlawtyConfig["channels"]>["telegram"]>>,
  ...
  configSpy.mockReturnValue({
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "klawty"),
        ...agentOverrides,
      },
      list: agentsList,
    },
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as KlawtyConfig },
    writeOptions: {},
