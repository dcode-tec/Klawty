...
function resolveAgentName(cfg: KlawtyConfig, agentId: string) {
...
function resolveAgentModel(cfg: KlawtyConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: KlawtyConfig): AgentSummary[] {
