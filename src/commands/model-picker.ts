...
type PromptDefaultModelResult = { model?: string; config?: KlawtyConfig };
...
function resolveConfiguredModelRaw(cfg: KlawtyConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: KlawtyConfig): string[] {
...
export function applyModelAllowlist(cfg: KlawtyConfig, models: string[]): KlawtyConfig {
