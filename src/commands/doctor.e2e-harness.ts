...
export const resolveKlawtyPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
export const resolveGatewayProgramArguments = vi.fn().mockResolvedValue({
  programArguments: ["node", "cli", "gateway", "--port", "2508"],
}) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadKlawtyPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveKlawtyPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.KLAWTY_STATE_DIR;
  originalUpdateInProgress = process.env.KLAWTY_UPDATE_IN_PROGRESS;
  process.env.KLAWTY_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "klawty-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.KLAWTY_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
