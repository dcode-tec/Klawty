...
type EnvSnapshot = {
  HOME?: string;
  KLAWTY_HOME?: string;
  KLAWTY_STATE_DIR?: string;
  KLAWTY_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    KLAWTY_HOME: process.env.KLAWTY_HOME,
    KLAWTY_STATE_DIR: process.env.KLAWTY_STATE_DIR,
    KLAWTY_OAUTH_DIR: process.env.KLAWTY_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: KlawtyConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: KlawtyConfig) {
...
async function runStateIntegrityText(cfg: KlawtyConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "klawty-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.KLAWTY_HOME = tempHome;
    process.env.KLAWTY_STATE_DIR = path.join(tempHome, ".klawty");
    delete process.env.KLAWTY_OAUTH_DIR;
    fs.mkdirSync(process.env.KLAWTY_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when KLAWTY_OAUTH_DIR is explicitly configured", async () => {
    process.env.KLAWTY_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: KlawtyConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints klawty-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: KlawtyConfig = {};
    ...
    expect(text).toMatch(/klawty sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/klawty sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /klawty sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
