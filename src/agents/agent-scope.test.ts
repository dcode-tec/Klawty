...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: KlawtyConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: KlawtyConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: KlawtyConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: KlawtyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: KlawtyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: KlawtyConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/klawty");
  });

  it("uses KLAWTY_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "klawty-home");
    vi.stubEnv("KLAWTY_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as KlawtyConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".klawty", "workspace"));
  });

  it("uses KLAWTY_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "klawty-home");
    vi.stubEnv("KLAWTY_HOME", home);
    // Clear state dir so it falls back to KLAWTY_HOME
    vi.stubEnv("KLAWTY_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as KlawtyConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".klawty", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/klawty-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/klawty-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "klawty-agent-scope-"));
