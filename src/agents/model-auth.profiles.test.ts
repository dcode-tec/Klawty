...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-oauth-"));

    try {
      const agentDir = path.join(tempDir, "agent");
      await withEnvAsync(
        {
          KLAWTY_STATE_DIR: tempDir,
          KLAWTY_AGENT_DIR: agentDir,
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.KLAWTY_AGENT_DIR, {
