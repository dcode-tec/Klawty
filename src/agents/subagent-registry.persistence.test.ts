...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["KLAWTY_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-subagent-"));
  ...
  it("uses isolated temp state when KLAWTY_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "klawty-test-state"));
  });
});
