...
function createOpenAiConfigWithResolvedApiKey(mergeMode = false): KlawtyConfig {
  return {
    models: {
...
describe("models-config", () => {
  it("keeps anthropic api defaults when model entries omit api", async () => {
    await withTempHome(async () => {
      ...
      await ensureKlawtyModelsJson(validated.config);
  ...
  it("merges providers by default", async () => {
    await withTempHome(async () => {
      ...
      await ensureKlawtyModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
