...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.KLAWTY_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".klawty", `workspace-${profile}`);
  }
  return path.join(home, ".klawty", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".klawty";
