...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const klawtyTmpDir = path.resolve(resolvePreferredKlawtyTmpDir());
  if (!isPathInside(klawtyTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: klawtyTmpDir });
  return resolved;
}
