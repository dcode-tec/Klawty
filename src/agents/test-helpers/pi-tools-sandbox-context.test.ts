...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("klawty-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("klawty-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("klawty-sbx-");
