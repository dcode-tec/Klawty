    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "klawty.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "klawty.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "klawty.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("klawty-");
  const acpxRoot = await tempDirs.make("klawty-acpx-plugin-");
  const helperRoot = await tempDirs.make("klawty-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("klawty-");
  const pluginRoot = await tempDirs.make("klawty-plugin-");
  const outsideDir = await tempDirs.make("klawty-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("klawty-");
    const pluginRoot = await tempDirs.make("klawty-claude-bundle-");
