...
const GOOGLE_LIVE_SINGLE_KEY = "KLAWTY_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "KLAWTY_LIVE_ANTHROPIC_KEY",
    listVar: "KLAWTY_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "KLAWTY_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `KLAWTY_LIVE_${base}_KEY`;
