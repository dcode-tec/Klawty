...
import { resolvePreferredKlawtyTmpDir } from "../infra/tmp-klawty-dir.js";
...
async function withOutsideHardlinkInKlawtyTmp<T>(
  params: {
    klawtyTmpDir: string;
  ...
  const hardlinkPath = path.join(params.klawtyTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.klawtyTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.klawtyTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.klawtyTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const klawtyTmpDir = resolvePreferredKlawtyTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Klawty tmp root",
      media: path.join(klawtyTmpDir, "image.png"),
      expected: path.join(klawtyTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Klawty tmp root",
      media: pathToFileURL(path.join(klawtyTmpDir, "photo.png")).href,
      expected: path.join(klawtyTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Klawty tmp root",
      media: path.join(klawtyTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(klawtyTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(klawtyTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside klawty tmp root",
      media: path.join(os.tmpdir(), "outside-klawty", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Klawty tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(klawtyTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(klawtyTmpDir, { recursive: true });
      const symlinkPath = path.join(klawtyTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked Klawty tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInKlawtyTmp(
      {
        klawtyTmpDir,
  ...
  it("rejects symlinked Klawty tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInKlawtyTmp(
