...
function resolveAttachmentLimits(config: KlawtyConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".klawty", "attachments");
  const relDir = path.posix.join(".klawty", "attachments", attachmentId);
