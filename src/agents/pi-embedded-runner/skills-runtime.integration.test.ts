...
const originalBundledDir = process.env.KLAWTY_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("klawty-bundled-");
  const workspaceDir = await createTempDir("klawty-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: KlawtyConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.KLAWTY_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
...
afterEach(async () => {
  process.env.KLAWTY_BUNDLED_PLUGINS_DIR = originalBundledDir;
  clearPluginManifestRegistryCache();
  await Promise.all(
