...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses KLAWTY_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      KLAWTY_HOME: "/srv/klawty-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/klawty-home"), ".klawty", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".klawty", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("klawty-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("klawty-workspace-");
    await fs.mkdir(path.join(tempDir, ".klawty"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-workspace-hardlink-"));
    try {
