...
import { markKlawtyExecEnv } from "../../infra/klawty-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "klawty.sandbox=1");
  args.push("--label", `klawty.sessionKey=${params.scopeKey}`);
  args.push("--label", `klawty.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `klawty.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markKlawtyExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "klawty.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`klawty sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`klawty sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("klawty sandbox recreate --all");
}
