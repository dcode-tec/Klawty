...
function findTool(name: string, config: KlawtyConfig) {
  const allTools = createKlawtyTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: KlawtyConfig; env?: NodeJS.ProcessEnv }) {
...
describe("klawty tools runtime web metadata wiring", () => {
