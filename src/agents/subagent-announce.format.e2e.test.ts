...
let configOverride: KlawtyConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: KlawtyConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/klawty/klawty/issues/31298
    previousFastTestEnv = process.env.KLAWTY_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.KLAWTY_TEST_FAST;
      return;
    }
    process.env.KLAWTY_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // KLAWTY_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Klawty runtime context (internal):");
