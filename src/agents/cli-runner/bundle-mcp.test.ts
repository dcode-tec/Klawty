...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a merged --mcp-config overlay for claude-cli", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("klawty-cli-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("klawty-cli-bundle-mcp-workspace-");
