 ...
 * A common mistake — caused by the similarity with the `klawty.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (klawty.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.KLAWTY_AUTH_STORE_READONLY === "1";
