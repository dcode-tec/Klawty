...
test("exec sets KLAWTY_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-klawty-shell", {
    command: "node -e \"process.stdout.write(process.env.KLAWTY_SHELL || '')\"",
    pty: true,
  });
