...
import type {
  ...
  KlawtyPluginApi,
  KlawtyPluginChannelRegistration,
  KlawtyPluginCliRegistrar,
  KlawtyPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  KlawtyPluginHttpRouteAuth,
  KlawtyPluginHttpRouteMatch,
  KlawtyPluginHttpRouteHandler,
  KlawtyPluginHttpRouteParams,
  KlawtyPluginHookOptions,
  MediaUnderstandingProviderPlugin,
  ProviderPlugin,
  KlawtyPluginService,
  KlawtyPluginToolContext,
  KlawtyPluginToolFactory,
  PluginConfigUiHint,
  PluginDiagnostic,
  PluginBundleFormat,
  PluginFormat,
...
export type PluginToolRegistration = {
  pluginId: string;
  pluginName?: string;
  factory: KlawtyPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
  rootDir?: string;
...
export type PluginCliRegistration = {
  pluginId: string;
  pluginName?: string;
  register: KlawtyPluginCliRegistrar;
  commands: string[];
  source: string;
  rootDir?: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: KlawtyPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: KlawtyPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: KlawtyPluginHookOptions | undefined,
    config: KlawtyPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: KlawtyPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: KlawtyPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as KlawtyPluginChannelRegistration).plugin === "object"
        ? (registration as KlawtyPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: KlawtyPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: KlawtyPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: KlawtyPluginCommandDefinition) => {
