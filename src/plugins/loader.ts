...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: KlawtyPluginDefinition;
  register?: KlawtyPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as KlawtyPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as KlawtyPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.KLAWTY_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverKlawtyPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverKlawtyPlugins>["candidates"][number];
  right: ReturnType<typeof discoverKlawtyPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadKlawtyPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadKlawtyPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  const getJiti = (modulePath: string) => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "klawty/plugin-sdk": pluginSdkAlias } : {}),
      ...resolvePluginSdkScopedAliasMap(),
    };
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into Klawty yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: KlawtyPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as KlawtyPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
