...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".klawty", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".klawty", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    ...
    const homeDir = path.resolve(path.sep, "tmp", "klawty-home");
    const roots = withEnv(
      {
        KLAWTY_BUNDLED_PLUGINS_DIR: path.join(ignoredHome, "ignored-bundled"),
        KLAWTY_STATE_DIR: path.join(ignoredHome, "ignored-state"),
        HOME: ignoredHome,
      },
      () =>
        resolvePluginSourceRoots({
          env: {
            ...process.env,
            HOME: homeDir,
            KLAWTY_BUNDLED_PLUGINS_DIR: "~/bundled",
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".klawty", "extensions"),
    });
  });
});
