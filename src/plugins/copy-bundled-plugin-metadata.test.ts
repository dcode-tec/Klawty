...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("klawty-bundled-plugin-meta-");
    ...
    writeJson(path.join(pluginDir, "klawty.plugin.json"), {
      id: "acpx",
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "klawty.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    const bundledManifest = JSON.parse(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "acpx", "klawty.plugin.json"),
        "utf8",
      ),
    ) as { skills?: string[] };
    ...
    ) as { klawty?: { extensions?: string[] } };
    expect(packageJson.klawty?.extensions).toEqual(["./index.js"]);
  });

  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("klawty-bundled-plugin-node-modules-");
    ...
    const bundledManifest = JSON.parse(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "tlon", "klawty.plugin.json"),
        "utf8",
      ),
    ) as { skills?: string[] };
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("klawty-bundled-plugin-hoisted-skill-");
    const pluginDir = path.join(repoRoot, "extensions", "tlon");
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("klawty-bundled-plugin-missing-skill-");
    const pluginDir = path.join(repoRoot, "extensions", "tlon");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "klawty.plugin.json"), {
      id: "tlon",
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("klawty-bundled-plugin-retry-");
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("klawty-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("klawty-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "klawty.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@klawty/google-gemini-cli-auth",
    });
