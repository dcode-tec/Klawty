...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): KlawtyConfig {
    const defaults: NonNullable<NonNullable<KlawtyConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: KlawtyConfig) {
