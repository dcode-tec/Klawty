...
const createTempDir = () => tempDirs.make("klawty-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets KLAWTY_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "klawty",
    });

    expect(env.KLAWTY_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("klawty");
  });

  it("overrides pre-existing KLAWTY_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      KLAWTY_SHELL: "wrong",
    });
    expect(env.KLAWTY_SHELL).toBe("acp-client");
  ...
  it("preserves KLAWTY_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        KLAWTY_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["KLAWTY_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Klawty bridge", () => {
    ...
    expect(env.KLAWTY_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.KLAWTY_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "klawty",
        serverArgs: ["acp"],
        defaultServerCommand: "klawty",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("KLAWTY_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "klawty", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "klawty", "dist", "entry.js");
    const shimPath = path.join(dir, "klawty.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\klawty\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/klawty-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/klawty-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/klawty-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
