...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("klawty-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("klawty-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("klawty-lid-a-", async (first) => {
      await withTempDir("klawty-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.klawty when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "klawty-config-dir-"));
    try {
  ...
  it("expands KLAWTY_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/klawty-home",
      KLAWTY_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/klawty-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers KLAWTY_HOME over HOME", () => {
    vi.stubEnv("KLAWTY_HOME", "/srv/klawty-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/klawty-home"));
...
describe("shortenHomePath", () => {
  it("uses $KLAWTY_HOME prefix when KLAWTY_HOME is set", () => {
    vi.stubEnv("KLAWTY_HOME", "/srv/klawty-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/klawty-home")}/.klawty/klawty.json`)).toBe(
      "$KLAWTY_HOME/.klawty/klawty.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $KLAWTY_HOME replacement when KLAWTY_HOME is set", () => {
    vi.stubEnv("KLAWTY_HOME", "/srv/klawty-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/klawty-home")}/.klawty/klawty.json`),
    ).toBe("config: $KLAWTY_HOME/.klawty/klawty.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/klawty", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "klawty"),
    );
  });
  ...
  it("prefers KLAWTY_HOME for tilde expansion", () => {
    vi.stubEnv("KLAWTY_HOME", "/srv/klawty-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/klawty")).toBe(path.resolve("/srv/klawty-home", "klawty"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/klawty", env)).toBe(path.resolve("/srv/klawty-home", "klawty"));
  });
