...
struct ChatMessageBubble: View {
    let message: KlawtyChatMessage
    let style: KlawtyChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? KlawtyChatTheme.userText : KlawtyChatTheme.assistantText
    ...
    private var inlineAttachments: [KlawtyChatMessageContent] {
    ...
    private var toolCalls: [KlawtyChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [KlawtyChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? KlawtyChatTheme.userBubble
        }
        if self.style == .onboarding {
            return KlawtyChatTheme.onboardingAssistantBubble
        }
        return KlawtyChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return KlawtyChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: KlawtyChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? KlawtyChatTheme.userText : KlawtyChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: KlawtyChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(KlawtyChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [KlawtyChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: KlawtyChatTheme.assistantText)
            }
        }
    }
}
