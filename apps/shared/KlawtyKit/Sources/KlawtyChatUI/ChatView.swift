...
@MainActor
public struct KlawtyChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: KlawtyChatViewModel
    ...
    private var visibleMessages: [KlawtyChatMessage] {
        let base: [KlawtyChatMessage]
    ...
    private func mergeToolResults(in messages: [KlawtyChatMessage]) -> [KlawtyChatMessage] {
    ...
    private func isToolResultMessage(_ message: KlawtyChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: KlawtyChatMessage) -> Bool {
    ...
    private func primaryText(in message: KlawtyChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: KlawtyChatMessage) -> Bool {
    ...
    private func toolCalls(in message: KlawtyChatMessage) -> [KlawtyChatMessageContent] {
    ...
    private func inlineToolResults(in message: KlawtyChatMessage) -> [KlawtyChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: KlawtyChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: KlawtyChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(KlawtyChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
