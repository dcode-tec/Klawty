...
private let chatUILogger = Logger(subsystem: "ai.klawty", category: "KlawtyChatUI")

@MainActor
@Observable
public final class KlawtyChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [KlawtyChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [KlawtyChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [KlawtyPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [KlawtyChatPendingToolCall] = []
    public private(set) var sessions: [KlawtyChatSessionEntry] = []
    private let transport: any KlawtyChatTransport
    private var sessionDefaults: KlawtyChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: KlawtyChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any KlawtyChatTransport,
    ...
    public var sessionChoices: [KlawtyChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [KlawtyChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: KlawtyPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [KlawtyChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: KlawtyChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: KlawtyChatMessage) -> KlawtyChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> KlawtyChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return KlawtyChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: KlawtyChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [KlawtyChatMessage],
        incoming: [KlawtyChatMessage]) -> [KlawtyChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [KlawtyChatMessage]) -> [KlawtyChatMessage] {
        var result: [KlawtyChatMessage] = []
    ...
    private static func dedupeKey(for message: KlawtyChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [KlawtyChatMessageContent] = [
            KlawtyChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> KlawtyChatAttachmentPayload in
            KlawtyChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> KlawtyChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = KlawtyChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: KlawtyChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: KlawtyChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: KlawtyAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = KlawtyChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> KlawtyPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
