...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> KlawtyChatHistoryPayload
{
    KlawtyChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> KlawtyChatSessionEntry {
    KlawtyChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> KlawtyChatSessionEntry
{
    KlawtyChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> KlawtyChatModelChoice {
    KlawtyChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [KlawtyChatHistoryPayload],
    sessionsResponses: [KlawtyChatSessionsListResponse] = [],
    modelResponses: [[KlawtyChatModelChoice]] = [],
    ...
    -> (TestChatTransport, KlawtyChatViewModel)
{
...
private func sendUserMessage(_ vm: KlawtyChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, KlawtyChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [KlawtyChatHistoryPayload]
    private let sessionsResponses: [KlawtyChatSessionsListResponse]
    private let modelResponses: [[KlawtyChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<KlawtyChatTransportEvent>
    private let continuation: AsyncStream<KlawtyChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<KlawtyChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<KlawtyChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> KlawtyChatHistoryPayload {
        ...
        return self.historyResponses.last ?? KlawtyChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [KlawtyChatAttachmentPayload]) async throws -> KlawtyChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return KlawtyChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> KlawtyChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? KlawtyChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [KlawtyChatModelChoice] {
    ...
    func emit(_ evt: KlawtyChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = KlawtyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = KlawtyChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: KlawtyChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = KlawtyChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: KlawtyChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(KlawtyChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == KlawtyChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = KlawtyChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: KlawtyChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = KlawtyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = KlawtyChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = KlawtyChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { KlawtyChatViewModel(sessionKey: "main", transport: transport) }
