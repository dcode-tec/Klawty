...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.klawty.mac`) even if the shipped
    ...
    {
        if environment["KLAWTY_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "klawty.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "klawty.nixMode") { return true }
