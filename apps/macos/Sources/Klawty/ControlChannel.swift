...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: KlawtyProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.klawty", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: KlawtyKit.AnyCodable]()) {
                $0[$1.key] = KlawtyKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: KlawtyProtocol.AnyCodable?) -> [String: KlawtyProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: KlawtyProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: KlawtyKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: KlawtyProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("klawty.control.heartbeat")
    static let controlAgentEvent = Notification.Name("klawty.control.agent")
}
