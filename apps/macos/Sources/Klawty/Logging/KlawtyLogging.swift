...
enum KlawtyLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = KlawtyOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = KlawtyFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        KlawtyLogging.bootstrapIfNeeded()
        let label = KlawtyLogging.makeLabel(subsystem: subsystem, category: category)
...
struct KlawtyOSLogHandler: AppLogLevelBackedHandler {
...
struct KlawtyFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = KlawtyLogging.parseLabel(self.label)
