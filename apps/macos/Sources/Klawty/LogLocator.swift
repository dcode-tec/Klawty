...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["KLAWTY_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/klawty")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("klawty-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("klawty-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/klawty/ (rolling or stdout), or nil if none exist.
