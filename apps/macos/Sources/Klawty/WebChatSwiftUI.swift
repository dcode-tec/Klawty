...
private let webChatSwiftLogger = Logger(subsystem: "ai.klawty", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "klawty.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: KlawtyChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> KlawtyChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(KlawtyChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            KlawtyChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return KlawtyChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [KlawtyChatAttachmentPayload]) async throws -> KlawtyChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> KlawtyChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                KlawtyGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: KlawtyProtocol.ModelChoice) -> KlawtyChatModelChoice {
        KlawtyChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<KlawtyChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any KlawtyChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = KlawtyChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: KlawtyChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<KlawtyChatView>) -> NSViewController
    {
