...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.klawty/klawty.json. Prefer KLAWTY_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.klawty/klawty.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.klawty/klawty.json. Restart the gateway to apply."
        }
