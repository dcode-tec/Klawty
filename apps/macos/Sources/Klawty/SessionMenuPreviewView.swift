...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.klawty", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> KlawtySessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: KlawtySessionPreviewEntry,
    ...
    private static func cache(payload: KlawtySessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: KlawtyChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [KlawtyKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [KlawtyKit.AnyCodable]) -> [KlawtyChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(KlawtyChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: KlawtyChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: KlawtyChatMessage) -> Bool {
    ...
    private static func toolNames(for message: KlawtyChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: KlawtyChatMessage) -> String? {
