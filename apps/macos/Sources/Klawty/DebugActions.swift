...
enum DebugActions {
    private static let verboseDefaultsKey = "klawty.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "klawty.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = KlawtyPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Klawty", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = KlawtyPaths.configURL
        guard
