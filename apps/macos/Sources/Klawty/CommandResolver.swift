...
enum CommandResolver {
    private static let projectRootDefaultsKey = "klawty.gatewayProjectRootPath"
    private static let helperName = "klawty"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let klawtyEntry = root.appendingPathComponent("klawty.mjs").path
        if FileManager().isReadableFile(atPath: klawtyEntry) { return klawtyEntry }
        let binEntry = root.appendingPathComponent("bin/klawty.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/klawty")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let klawtyPaths = self.klawtyManagedPaths(home: home)
        if !klawtyPaths.isEmpty {
            extras.insert(contentsOf: klawtyPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + klawtyPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func klawtyManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".klawty"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func klawtyExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectKlawtyExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("klawty.mjs").path,
            root.appendingPathComponent("bin/klawty.js").path,
        ]
    ...
    static func hasAnyKlawtyInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.klawtyExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let klawtyPath = self.projectKlawtyExecutable(projectRoot: root) {
            return [klawtyPath, subcommand] + extraArgs
        }
        if let klawtyPath = self.klawtyExecutable(searchPaths: searchPaths) {
            return [klawtyPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "klawty", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            klawty entrypoint missing (looked for dist/index.js or klawty.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real klawty CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/klawty"
        ...
        \(cliSection)
        \(projectSection)
        if command -v klawty >/dev/null 2>&1; then
          CLI="$(command -v klawty)"
          klawty \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/klawty.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/klawty.mjs"
            node "$PRJ/klawty.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/klawty.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/klawty.js"
            node "$PRJ/bin/klawty.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent klawty"
          pnpm --silent klawty \(quotedArgs);
        else
          echo "klawty CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? KlawtyConfigFile.loadDict()
