...
struct KlawtyMacCLI {
    static func main() async {
        ...
        default:
            fputs("klawty-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      klawty-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      klawty-mac discover [--timeout <ms>] [--json] [--include-local]
      klawty-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      klawty-mac connect
      klawty-mac connect --url ws://127.0.0.1:18789 --json
      klawty-mac discover --timeout 3000 --json
      klawty-mac wizard --mode local
    """)
}
