...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: KlawtyProtocol.AnyCodable(["ok": KlawtyProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: KlawtyProtocol.AnyCodable(["ok": KlawtyProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = KlawtyProtocol.AnyCodable([
            "runId": KlawtyProtocol.AnyCodable("run-1"),
            "sessionKey": KlawtyProtocol.AnyCodable("main"),
            "state": KlawtyProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: KlawtyProtocol.AnyCodable(["a": KlawtyProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
