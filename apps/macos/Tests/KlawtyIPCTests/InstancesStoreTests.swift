...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: KlawtyProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: KlawtyProtocol.AnyCodable] = [
            "presence": .init([KlawtyProtocol.AnyCodable(entry)]),
        ]
        let payload = KlawtyProtocol.AnyCodable(payloadMap)
