...
private const val NOTIFICATION_CHANNEL_BASE_ID = "klawty.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Klawty Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Klawty Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Klawty Active")
      }
