...
@MainActor
final class KlawtyAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.klawty.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.klawty.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.klawty.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "klawty.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "klawty.watch.promptId"
    static let sessionKeyKey = "klawty.watch.sessionKey"
    static let actionPrimaryIDKey = "klawty.watch.action.primary.id"
    static let actionPrimaryLabelKey = "klawty.watch.action.primary.label"
    static let actionSecondaryIDKey = "klawty.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "klawty.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "klawty.watch.action.primary"
    static let actionSecondaryIdentifier = "klawty.watch.action.secondary"
    static let actionIdentifierPrefix = "klawty.watch.action."
    static let actionIDKeyPrefix = "klawty.watch.action.id."
    static let actionLabelKeyPrefix = "klawty.watch.action.label."
    static let categoryPrefix = "klawty.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: KlawtyWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> KlawtyWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return KlawtyWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Klawty" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [KlawtyWatchAction]) -> [UNNotificationAction] {
...
@main
struct KlawtyApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(KlawtyAppDelegate.self) private var appDelegate
...
extension KlawtyApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Klawty: installing uncaught exception handler")
