...
final class ContactsService: ContactsServicing {
    ...
    func search(params: KlawtyContactsSearchParams) async throws -> KlawtyContactsSearchPayload {
        ...
        return KlawtyContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: KlawtyContactsAddParams) async throws -> KlawtyContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return KlawtyContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return KlawtyContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> KlawtyContactPayload {
        KlawtyContactPayload(
