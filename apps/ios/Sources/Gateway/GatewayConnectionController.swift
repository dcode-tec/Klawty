...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [KlawtyCapability.canvas.rawValue, KlawtyCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(KlawtyCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(KlawtyCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = KlawtyLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(KlawtyCapability.location.rawValue) }

        caps.append(KlawtyCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(KlawtyCapability.watch.rawValue)
        }
        caps.append(KlawtyCapability.photos.rawValue)
        caps.append(KlawtyCapability.contacts.rawValue)
        caps.append(KlawtyCapability.calendar.rawValue)
        caps.append(KlawtyCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(KlawtyCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            KlawtyScreenCommand.record.rawValue,
            KlawtySystemCommand.notify.rawValue,
            KlawtyChatCommand.push.rawValue,
        ...
        if caps.contains(KlawtyCapability.camera.rawValue) {
            commands.append(KlawtyCameraCommand.list.rawValue)
            commands.append(KlawtyCameraCommand.snap.rawValue)
            commands.append(KlawtyCameraCommand.clip.rawValue)
        }
        if caps.contains(KlawtyCapability.location.rawValue) {
            commands.append(KlawtyLocationCommand.get.rawValue)
        }
        if caps.contains(KlawtyCapability.device.rawValue) {
            commands.append(KlawtyDeviceCommand.status.rawValue)
            commands.append(KlawtyDeviceCommand.info.rawValue)
        }
        if caps.contains(KlawtyCapability.watch.rawValue) {
            commands.append(KlawtyWatchCommand.status.rawValue)
            commands.append(KlawtyWatchCommand.notify.rawValue)
        }
        if caps.contains(KlawtyCapability.photos.rawValue) {
            commands.append(KlawtyPhotosCommand.latest.rawValue)
        }
        if caps.contains(KlawtyCapability.contacts.rawValue) {
            commands.append(KlawtyContactsCommand.search.rawValue)
            commands.append(KlawtyContactsCommand.add.rawValue)
        }
        if caps.contains(KlawtyCapability.calendar.rawValue) {
            commands.append(KlawtyCalendarCommand.events.rawValue)
            commands.append(KlawtyCalendarCommand.add.rawValue)
        }
        if caps.contains(KlawtyCapability.reminders.rawValue) {
            commands.append(KlawtyRemindersCommand.list.rawValue)
            commands.append(KlawtyRemindersCommand.add.rawValue)
        }
        if caps.contains(KlawtyCapability.motion.rawValue) {
            commands.append(KlawtyMotionCommand.activity.rawValue)
            commands.append(KlawtyMotionCommand.pedometer.rawValue)
        }
