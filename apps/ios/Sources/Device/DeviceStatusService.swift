...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> KlawtyDeviceStatusPayload {
        ...
        return KlawtyDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> KlawtyDeviceInfoPayload {
        ...
        return KlawtyDeviceInfoPayload(
    ...
    private func batteryStatus() -> KlawtyBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: KlawtyBatteryState = switch device.batteryState {
        ...
        return KlawtyBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> KlawtyThermalStatusPayload {
        let state: KlawtyThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return KlawtyThermalStatusPayload(state: state)
    }

    private func storageStatus() -> KlawtyStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return KlawtyStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
