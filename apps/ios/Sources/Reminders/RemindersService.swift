...
final class RemindersService: RemindersServicing {
    func list(params: KlawtyRemindersListParams) async throws -> KlawtyRemindersListPayload {
        ...
        let payload: [KlawtyReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return KlawtyRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: KlawtyRemindersAddParams) async throws -> KlawtyRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = KlawtyReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return KlawtyRemindersAddPayload(reminder: payload)
    }
