...
final class MotionService: MotionServicing {
    func activities(params: KlawtyMotionActivityParams) async throws -> KlawtyMotionActivityPayload {
        ...
        let mapped: [KlawtyMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return KlawtyMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: KlawtyPedometerParams) async throws -> KlawtyPedometerPayload {
        ...
        let payload: KlawtyPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = KlawtyPedometerPayload(
