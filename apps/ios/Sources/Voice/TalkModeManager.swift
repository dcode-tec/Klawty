...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__KLAWTY_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<KlawtyTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.klawty", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = KlawtyTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> KlawtyTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return KlawtyTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> KlawtyTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> KlawtyTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return KlawtyTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> KlawtyTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = KlawtyTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: KlawtyTalkPTTStopPayload) {
