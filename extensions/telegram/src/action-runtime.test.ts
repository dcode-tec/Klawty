...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): KlawtyConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as KlawtyConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): KlawtyConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as KlawtyConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: KlawtyConfig, accountId = "media") {
    await handleTelegramAction(
