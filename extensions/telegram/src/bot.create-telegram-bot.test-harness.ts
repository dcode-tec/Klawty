...
import { resetInboundDedupe } from "klawty/plugin-sdk/reply-runtime";
import type { MsgContext } from "klawty/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "klawty/plugin-sdk/reply-runtime";
...
type LoadConfigFn = typeof import("klawty/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("klawty/plugin-sdk/config-runtime").resolveStorePath;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/klawty-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("klawty/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("klawty/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("klawty/plugin-sdk/config-runtime", async (importOriginal) => {
...
vi.doMock("klawty/plugin-sdk/conversation-runtime", async (importOriginal) => {
...
vi.doMock("klawty/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("klawty/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: KlawtyConfig): {
...
vi.doMock("klawty/plugin-sdk/reply-runtime", async (importOriginal) => {
...
vi.doMock("klawty/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("klawty/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("klawty/plugin-sdk/infra-runtime", async (importOriginal) => {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: KlawtyConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: KlawtyConfig) => {
