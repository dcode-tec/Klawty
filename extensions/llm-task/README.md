...
`klawty.invoke --each`) without adding new Klawty code per workflow.
...
This extension depends on Klawty internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Klawty extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.klawty/extensions` as a standalone plugin directory.
