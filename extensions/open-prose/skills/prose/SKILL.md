...
metadata: { "klawty": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Klawty `sessions_spawn`
- **File I/O** == Klawty `read`/`write`
- **Remote fetch** == Klawty `web_fetch` (or `exec` with curl when POST is required)
