...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Klawty runtime is using the same account. " +
    `Check 'klawty matrix verify status --verbose --account ${accountId}' and 'klawty matrix devices list --account ${accountId}'.`
  );
}
