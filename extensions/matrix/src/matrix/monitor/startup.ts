...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleKlawtyDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Klawty devices detected for ${params.auth.userId}: ${deviceHealth.staleKlawtyDevices.map((device) => device.deviceId).join(", ")}. Run 'klawty matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
