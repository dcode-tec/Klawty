...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<KlawtyPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: KlawtyPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<KlawtyPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as KlawtyPluginApi);
