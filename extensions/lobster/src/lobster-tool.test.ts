...
import type { KlawtyPluginApi, KlawtyPluginToolContext } from "../runtime-api.js";
import {
...
function fakeApi(overrides: Partial<KlawtyPluginApi> = {}): KlawtyPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<KlawtyPluginToolContext> = {}): KlawtyPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "klawty-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: KlawtyPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
