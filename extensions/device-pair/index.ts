...
function resolveGatewayPort(cfg: KlawtyPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.KLAWTY_GATEWAY_PORT?.trim()) ??
...
function resolveAuthLabel(cfg: KlawtyPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: KlawtyPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for Klawty devices",
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (channel === "telegram" && target) {
            try {
              ...
              if (send) {
                await send(
                  target,
                  ["Scan this QR code with the Klawty iOS app:", "", "```", qrAscii, "```"].join(
                    "\n",
                  ),
                  {
          ...
          return {
            text: [
              "Scan this QR code with the Klawty iOS app:",
              "",
              "```",
              qrAscii,
              "```",
