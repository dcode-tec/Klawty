...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked KLAWTY_HOME.
  delete process.env.KLAWTY_HOME;
  process.env.KLAWTY_STATE_DIR = path.join(base, ".klawty");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "klawty-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".klawty", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
