...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `klawty-e2e-${name}-`));
  const configDir = path.join(homeDir, ".klawty");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          KLAWTY_SKIP_PROVIDERS: "1",
          KLAWTY_SKIP_GMAIL_WATCHER: "1",
          KLAWTY_SKIP_CRON: "1",
          KLAWTY_SKIP_BROWSER_CONTROL_SERVER: "1",
          KLAWTY_SKIP_CANVAS_HOST: "1",
          KLAWTY_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
