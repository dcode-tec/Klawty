...
process.env.KLAWTY_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime Klawty modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: KlawtyConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: KlawtyConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
