...
CLI onboarding is the **recommended** way to set up Klawty on macOS,
...
`klawty dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`klawty configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.klawty/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `klawty doctor` first.
</Note>
...
Use `klawty agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.klawty/workspace-<agentId>`.
...
- CLI command reference: [`klawty onboard`](/cli/onboard)
