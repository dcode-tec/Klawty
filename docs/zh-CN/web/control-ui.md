...
- 默认：`http://<host>:2508/`
- 可选前缀：设置 `gateway.controlUi.basePath`（例如 `/klawty`）
...
- http://127.0.0.1:2508/（或 http://localhost:2508/）

如果页面加载失败，请先启动 Gateway 网关：`klawty gateway`。
...
一旦批准，设备会被记住，除非你使用 `klawty devices revoke --device <id> --role <role>` 撤销它，否则不需要重新批准。参见 [Devices CLI](/cli/devices) 了解 token 轮换和撤销。
...
- 配置：查看/编辑 `~/.klawty/klawty.json`（`config.get`、`config.set`）
...
- `http://<tailscale-ip>:2508/`（或你配置的 `gateway.controlUi.basePath`）

将 token 粘贴到 UI 设置中（作为 `connect.params.auth.token` 发送）。

## 不安全的 HTTP

如果你通过普通 HTTP 打开仪表板（`http://<lan-ip>` 或 `http://<tailscale-ip>`），浏览器在**非安全上下文**中运行并阻止 WebCrypto。默认情况下，Klawty **阻止**没有设备身份的控制 UI 连接。

**推荐修复：** 使用 HTTPS（Tailscale Serve）或在本地打开 UI：

- `https://<magicdns>/`（Serve）
- `http://127.0.0.1:2508/`（在 Gateway 网关主机上）
...
然后将 UI 指向你的 Gateway 网关 WS URL（例如 `ws://127.0.0.1:2508`）。
