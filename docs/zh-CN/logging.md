...
你可以在 `~/.klawty/klawty.json` 中覆盖此设置：
...
所有日志配置都在 `~/.klawty/klawty.json` 的 `logging` 下。
...
- Klawty 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `klawty plugins enable diagnostics-otel` 启用插件。
...
- `klawty.tokens`（计数器，属性：`klawty.token`、`klawty.channel`、`klawty.provider`、`klawty.model`）
- `klawty.cost.usd`（计数器，属性：`klawty.channel`、`klawty.provider`、`klawty.model`）
- `klawty.run.duration_ms`（直方图，属性：`klawty.channel`、`klawty.provider`、`klawty.model`）
- `klawty.context.tokens`（直方图，属性：`klawty.context`、`klawty.channel`、`klawty.provider`、`klawty.model`）

消息流：

- `klawty.webhook.received`（计数器，属性：`klawty.channel`、`klawty.webhook`）
- `klawty.webhook.error`（计数器，属性：`klawty.channel`、`klawty.webhook`）
- `klawty.webhook.duration_ms`（直方图，属性：`klawty.channel`、`klawty.webhook`）
- `klawty.message.queued`（计数器，属性：`klawty.channel`、`klawty.source`）
- `klawty.message.processed`（计数器，属性：`klawty.channel`、`klawty.outcome`）
- `klawty.message.duration_ms`（直方图，属性：`klawty.channel`、`klawty.outcome`）

队列 + 会话：

- `klawty.queue.lane.enqueue`（计数器，属性：`klawty.lane`）
- `klawty.queue.lane.dequeue`（计数器，属性：`klawty.lane`）
- `klawty.queue.depth`（直方图，属性：`klawty.lane` 或 `klawty.channel=heartbeat`）
- `klawty.queue.wait_ms`（直方图，属性：`klawty.lane`）
- `klawty.session.state`（计数器，属性：`klawty.state`、`klawty.reason`）
- `klawty.session.stuck`（计数器，属性：`klawty.state`）
- `klawty.session.stuck_age_ms`（直方图，属性：`klawty.state`）
- `klawty.run.attempt`（计数器，属性：`klawty.attempt`）
...
- `klawty.webhook.error`
  - `klawty.channel`、`klawty.webhook`、`klawty.chatId`、`klawty.error`
- `klawty.message.processed`
  - `klawty.channel`、`klawty.outcome`、`klawty.chatId`、`klawty.messageId`、`klawty.sessionKey`、`klawty.sessionId`、`klawty.reason`
- `klawty.session.stuck`
  - `klawty.state`、`klawty.ageMs`、`klawty.queueDepth`、`klawty.sessionKey`、`klawty.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `klawty doctor`。
