...
- `~/Library/Application Support/Klawty/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `klawty-canvas://<session>/<path>`

示例：

- `klawty-canvas://main/` → `<canvasRoot>/main/index.html`
- `klawty-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `klawty-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
klawty nodes canvas present --node <id>
klawty nodes canvas navigate --node <id> --url "/"
klawty nodes canvas eval --node <id> --js "document.title"
klawty nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__klawty__/a2ui/
```
...
klawty nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
klawty nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "klawty://agent?message=Review%20this%20design";
```
