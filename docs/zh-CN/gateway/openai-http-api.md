...
Klawty 的 Gateway 网关可以提供一个小型的 OpenAI 兼容 Chat Completions 端点。
...
底层实现中，请求作为普通的 Gateway 网关智能体运行执行（与 `klawty agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- 当 `gateway.auth.mode="token"` 时，使用 `gateway.auth.token`（或 `KLAWTY_GATEWAY_TOKEN`）。
- 当 `gateway.auth.mode="password"` 时，使用 `gateway.auth.password`（或 `KLAWTY_GATEWAY_PASSWORD`）。

## 选择智能体

无需自定义头：在 OpenAI `model` 字段中编码智能体 ID：

- `model: "klawty:<agentId>"`（例如：`"klawty:main"`、`"klawty:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过头指定特定的 Klawty 智能体：

- `x-klawty-agent-id: <agentId>`（默认：`main`）

高级选项：

- `x-klawty-session-key: <sessionKey>` 完全控制会话路由。
