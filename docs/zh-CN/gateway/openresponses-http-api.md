...
Klawty 的 Gateway 网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为正常的 Gateway 网关智能体运行执行（与 `klawty agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- `model: "klawty:<agentId>"`（示例：`"klawty:main"`、`"klawty:beta"`）
...
```bash
curl -sS http://127.0.0.1:2508/v1/responses \
...
```bash
curl -N http://127.0.0.1:2508/v1/responses \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -H 'Content-Type: application/json' \
  -H 'x-klawty-agent-id: main' \
  -d '{
