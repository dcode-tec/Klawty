...
service via the external `klawty` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.klawty.gateway`
  (or `ai.klawty.<profile>` when using `--profile`/`KLAWTY_PROFILE`; legacy `com.klawty.*` is supported).
