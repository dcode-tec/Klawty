...
- Same path: run the app from a fixed location (for Klawty, `dist/Klawty.app`).
...
```bash
sudo tccutil reset Accessibility ai.klawty.mac
sudo tccutil reset ScreenCapture ai.klawty.mac
...
Workaround: move files into the Klawty workspace (`~/.klawty/workspace`) if you want to avoid per-folder grants.
