...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `klawty.internal.`), **or**
...
- `KLAWTY_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Klawty iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Klawty builds.
...
```bash
export KLAWTY_APNS_TEAM_ID="TEAMID"
export KLAWTY_APNS_KEY_ID="KEYID"
export KLAWTY_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_klawty-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `klawty.internal.`) and Tailscale split DNS.
See [Bonjour](/gateway/bonjour) for the CoreDNS example.

### Manual host/port

In Settings, enable **Manual Host** and enter the gateway host + port (default `2508`).
...
```bash
klawty nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:2508/__klawty__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__klawty__/canvas/` and `/__klawty__/a2ui/`.
- It is served from the Gateway HTTP server (same port as `gateway.port`, default `2508`).
...
```bash
klawty nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__klawty; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
klawty nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `klawty devices list` and approve manually.
