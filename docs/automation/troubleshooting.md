...
klawty system heartbeat last
klawty logs --follow
klawty config get agents.defaults.heartbeat
klawty channels status --probe
```
...
```bash
klawty config get agents.defaults.heartbeat.activeHours
klawty config get agents.defaults.heartbeat.activeHours.timezone
klawty config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
klawty cron list
klawty logs --follow
```
