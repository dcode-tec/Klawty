...
read_when:
  ...
  - Wiring external systems into Klawty
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Klawty config directory (typically `~/.klawty/hooks/transforms`).
...
- `klawty webhooks gmail setup` writes `hooks.gmail` config for `klawty webhooks gmail run`.
...
```bash
curl -X POST http://127.0.0.1:2508/hooks/wake \
...
```bash
curl -X POST http://127.0.0.1:2508/hooks/agent \
  -H 'x-klawty-token: SECRET' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST http://127.0.0.1:2508/hooks/gmail \
  -H 'Authorization: Bearer SECRET' \
  -H 'Content-Type: application/json' \
