---
summary: "Use Venice AI privacy-focused models in Klawty"
read_when:
  - You want privacy-focused inference in Klawty
...
```bash
klawty onboard --auth-choice venice-api-key
```
...
```bash
klawty agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Klawty shows all available Venice models. Pick based on your needs:
...
```bash
klawty models set venice/kimi-k2-5
klawty models set venice/claude-opus-4-6
```

List all available models:

```bash
klawty models list | grep venice
```

## Configure via `klawty configure`

1. Run `klawty configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Klawty automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
klawty agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
klawty agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
klawty agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
klawty agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
klawty agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `klawty models list` to see currently available models. Some models may be temporarily offline.
