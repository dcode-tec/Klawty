---
summary: "Use Synthetic's Anthropic-compatible API in Klawty"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Klawty registers it as the
...
```bash
klawty onboard --auth-choice synthetic-api-key
```
...
Note: Klawty's Anthropic client appends `/v1` to the base URL, so use
