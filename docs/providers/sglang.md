---
summary: "Run Klawty with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Klawty against a local SGLang server
...
Klawty can connect to SGLang using the `openai-completions` API.

Klawty can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Klawty will query:
