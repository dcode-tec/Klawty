---
summary: "Use OpenCode Zen and Go catalogs with Klawty"
read_when:
...
OpenCode exposes two hosted catalogs in Klawty:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Klawty keeps the runtime provider ids
...
klawty onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
klawty onboard --auth-choice opencode-go
klawty onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
