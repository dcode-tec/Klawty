---
summary: "Use MiniMax M2.5 in Klawty"
read_when:
  - You want MiniMax models in Klawty
...
```bash
klawty plugins enable minimax  # skip if already loaded.
klawty gateway restart  # restart if gateway is already running
klawty onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/klawty/klawty/tree/main/extensions/minimax) for details.
...
Configure manually via `klawty.json`:
...
- Use `klawty models list` and `klawty models set minimax/MiniMax-M2.5` to switch.
...
- Running `klawty configure` and selecting **MiniMax M2.5**, or
