...
Klawty supports **camera capture** for agent workflows:
...
```bash
klawty nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
klawty nodes camera snap --node <id> --facing front
klawty nodes camera clip --node <id> --duration 3000
klawty nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`klawty.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `klawty` CLI to invoke camera commands on the macOS node.

Examples:

```bash
klawty nodes camera list --node <id>            # list camera ids
klawty nodes camera snap --node <id>            # prints MEDIA:<path>
klawty nodes camera snap --node <id> --max-width 1280
klawty nodes camera snap --node <id> --delay-ms 2000
klawty nodes camera snap --node <id> --device-id <id>
klawty nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
klawty nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
klawty nodes camera clip --node <id> --device-id <id>
klawty nodes camera clip --node <id> --no-audio
```

Notes:

- `klawty nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
klawty nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
