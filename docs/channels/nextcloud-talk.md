...
```bash
klawty plugins install @klawty/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
klawty plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Klawty" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `klawty pairing list nextcloud-talk`
  - `klawty pairing approve nextcloud-talk <CODE>`
