---
summary: "Mattermost bot setup and Klawty config"
read_when:
...
```bash
klawty plugins install @klawty/mattermost
```

Local checkout (when running from a git repo):

```bash
klawty plugins install ./extensions/mattermost
```
...
4. Configure Klawty and start the gateway.
...
Native slash commands are opt-in. When enabled, Klawty registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Klawty derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Klawty.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Klawty.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Klawty, not `404`.
...
- Approve via:
  - `klawty pairing list mattermost`
  - `klawty pairing approve mattermost <CODE>`
...
Use these target formats with `klawty message send` or cron/webhooks:
...
Klawty resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Klawty sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Klawty sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Klawty derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Klawty run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="klawty-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"klawty-mattermost-interactions",
...
`klawty message send` and cron/webhook deliveries.
