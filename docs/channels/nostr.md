...
read_when:
  - You want Klawty to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Klawty to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`klawty onboard`) and `klawty channels add` list optional channel plugins.
...
```bash
klawty plugins install @klawty/nostr
```

Use a local checkout (dev workflows):

```bash
klawty plugins install --link <path-to-klawty>/extensions/nostr
```
...
klawty channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "klawty@example.com",
        "lud16": "klawty@example.com"
      }
    }
  }
}
