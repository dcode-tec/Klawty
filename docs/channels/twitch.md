...
read_when:
  - Setting up Twitch chat integration for Klawty
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Klawty connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
klawty plugins install @klawty/twitch
```

Local checkout (when running from a git repo):

```bash
klawty plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `KLAWTY_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "klawty", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use KLAWTY_TWITCH_ACCESS_TOKEN env var)
...
```bash
KLAWTY_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
