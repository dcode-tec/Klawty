...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Klawty HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `klawty pairing approve slack <code>`.
...
- The interactive callback values are Klawty-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Klawty falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Klawty is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Klawty",
    "description": "Slack connector for Klawty"
  },
  "features": {
    "bot_user": {
      "display_name": "Klawty",
    ...
    "slash_commands": [
      {
        "command": "/klawty",
        "description": "Send a message to Klawty",
        "should_escape": false
      }
    ]
  },
...
Klawty supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Klawty falls back to normal delivery for remaining payloads.
