...
read_when:
  - Setting up Zalo Personal for Klawty
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Klawty.
...
- Install via CLI: `klawty plugins install @klawty/zalouser`
- Or from a source checkout: `klawty plugins install ./extensions/zalouser`
...
- `klawty pairing list zalouser`
- `klawty pairing approve zalouser <code>`
...
- On startup, Klawty resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Klawty stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Klawty state. Example:
...
- Klawty sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Klawty sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `klawty channels logout --channel zalouser && klawty channels login --channel zalouser`
...
- The channel now runs fully in Klawty without external CLI binaries.
