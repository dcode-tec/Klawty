---
summary: "Synology Chat webhook setup and Klawty config"
read_when:
  - Setting up Synology Chat with Klawty
...
```bash
klawty plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `klawty onboard` now shows Synology Chat in the same channel setup list as `klawty channels add`.
   - Non-interactive setup: `klawty channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Klawty gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Klawty.
   - Guided: `klawty onboard`
   - Direct: `klawty channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `klawty pairing list synology-chat`
  - `klawty pairing approve synology-chat <CODE>`
...
```bash
klawty message send --channel synology-chat --target 123456 --text "Hello from Klawty"
klawty message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
