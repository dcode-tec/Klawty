...
Tlon is a decentralized messenger built on Urbit. Klawty connects to your Urbit ship and can
...
```bash
klawty plugins install @klawty/tlon
```

Local checkout (when running from a git repo):

```bash
klawty plugins install ./extensions/tlon
```
...
By default, Klawty blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `klawty message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Klawty replies in-thread.
