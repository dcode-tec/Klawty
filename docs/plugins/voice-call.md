...
read_when:
  - You want to place an outbound voice call from Klawty
...
Voice calls for Klawty via a plugin. Supports outbound notifications and
...
- Use `klawty voicecall ...` or the `voice_call` tool
...
```bash
klawty plugins install ./extensions/voice-call
...
klawty voicecall start --to "+15555550123"   # alias for call
...
klawty voicecall latency                     # summarize turn latency from logs
