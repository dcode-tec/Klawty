...
read_when:
  - You are building an Klawty plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (klawty.plugin.json)

This page is for the **native Klawty plugin manifest** only.
...
Klawty auto-detects those bundle layouts too, but they are not validated
against the `klawty.plugin.json` schema described here.

For compatible bundles, Klawty currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Klawty runtime expectations.

Every native Klawty plugin **must** ship a `klawty.plugin.json` file in the
**plugin root**. Klawty uses this manifest to validate configuration
...
- `providerAuthEnvVars` (object): auth env vars keyed by provider id. Use this
  when Klawty should resolve provider credentials from env without loading
  plugin runtime first.
- `providerAuthChoices` (array): cheap onboarding/auth-choice metadata keyed by
  provider + auth method. Use this when Klawty should show a provider in
...
- The manifest is **required for native Klawty plugins**, including local filesystem loads.
