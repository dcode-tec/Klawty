...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `klawty sandbox explain`.
...
Legacy `agent.*` configs are migrated by `klawty doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=klawty-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${KLAWTY_STATE_DIR:-$HOME/.klawty}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
