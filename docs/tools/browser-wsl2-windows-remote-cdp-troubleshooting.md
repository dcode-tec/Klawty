...
read_when:
  - Running Klawty Gateway in WSL2 while Chrome lives on Windows
...
- Klawty Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/klawty/klawty/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Klawty and Chrome are on the same machine
...
- WSL2 runs the Gateway on `127.0.0.1:2508`
- Windows opens the Control UI in a normal browser at `http://127.0.0.1:2508/`
- Windows Chrome exposes a CDP endpoint on port `9222`
- WSL2 can reach that Windows CDP endpoint
- Klawty points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Klawty is not the problem yet.
...
Fix that before touching Klawty config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Klawty at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Klawty to succeed
...
```bash
klawty browser open https://example.com --browser-profile remote
klawty browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `klawty browser tabs` returns the target
...
3. Klawty config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
4. Control UI: are you opening `http://127.0.0.1:2508/` instead of a LAN IP?
...
- only then debug Klawty config or Control UI auth
