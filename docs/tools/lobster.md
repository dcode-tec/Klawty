---
title: Lobster
summary: "Typed workflow runtime for Klawty with resumable approval gates."
description: Typed workflow runtime for Klawty — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Klawty run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Klawty runs one Lobster tool call and gets a structured result.
...
Klawty launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | klawty.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
klawty.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Klawty tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Klawty Gateway (see the [Lobster repo](https://github.com/klawty/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Klawty keeps core tools enabled. To restrict core
...
→ klawty calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ klawty calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Klawty tools that do.
