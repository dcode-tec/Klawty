...
- **gateway host** → `klawty` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Klawty also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.klawty/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.klawty/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `klawty approvals allowlist ...`).
- `klawty security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `klawty doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.klawty/exec-approvals.json` directly.

CLI: `klawty approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Klawty does not treat
...
topics, Klawty preserves the topic for the approval prompt and the post-approval follow-up.
