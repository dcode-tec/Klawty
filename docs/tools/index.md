---
summary: "Agent tool surface for Klawty (browser, canvas, nodes, message, cron) replacing legacy `klawty-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `klawty-*` skills
title: "Tools"
---

# Tools (Klawty)

Klawty exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `klawty-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `klawty.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Klawty logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:klawty`: all built-in Klawty tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`klawty node run`).
...
Klawty tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `klawty configure --section web`).
...
Control the dedicated Klawty-managed browser.
...
- Omit `profile` for the safe default: isolated Klawty-managed browser (`klawty`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `klawty`).
...
- Quick smoke: `klawty nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- Available when `agents.defaults.imageGenerationModel` is configured, or when Klawty can infer a compatible image-generation default from your enabled providers plus available auth.
...
- When editing and `resolution` is omitted, Klawty infers a draft/final resolution from the input image size.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `klawty gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Klawty uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.klawty/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Klawty clamps `tools.sessions.visibility` to `tree`.
...
- `gatewayUrl` (default `ws://127.0.0.1:2508`)
