...
Klawty can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `klawty configure --section web` enables the bundled Firecrawl plugin automatically.
...
Klawty always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
