...
This guide summarizes a sane workflow for working on the pi integration in Klawty.
...
```bash
KLAWTY_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm klawty agent --message "Hello" --thinking low`
...
State lives under the Klawty state directory. Default is `~/.klawty`. If `KLAWTY_STATE_DIR` is set, use that directory instead.

To reset everything:

- `klawty.json` for config
