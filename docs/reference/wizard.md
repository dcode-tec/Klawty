...
This is the full reference for `klawty onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.klawty/klawty.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `klawty doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.klawty/credentials/oauth.json`; auth profiles live in `~/.klawty/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.klawty/credentials/oauth.json` (or `$KLAWTY_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.klawty/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `klawty pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `klawty configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `klawty health`.
    - Tip: `klawty status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export KLAWTY_GATEWAY_TOKEN="your-token"
klawty onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env KLAWTY_GATEWAY_TOKEN
```
...
- Stores it under `~/.klawty/tools/signal-cli/<version>/`.
...
Typical fields in `~/.klawty/klawty.json`:
...
`klawty agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.klawty/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.klawty/agents/<agentId>/sessions/`.
