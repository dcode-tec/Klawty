---
summary: "How Klawty builds prompt context and reports token usage + costs"
read_when:
...
Klawty tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Klawty assembles its own system prompt on every run. It includes:
...
For images, Klawty downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Klawty session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `klawty status --usage` and `klawty channels list` show
...
`cacheWrite`. If pricing is missing, Klawty shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Klawty can
...
Anthropic's 1M context window is currently beta-gated. Klawty can inject the
...
Klawty skips the `context-1m-*` beta header because Anthropic currently
