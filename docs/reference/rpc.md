...
Klawty integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Klawty owns lifecycle when `channels.signal.autoStart=true`.
...
- Klawty spawns `imsg rpc` as a child process (legacy iMessage integration).
