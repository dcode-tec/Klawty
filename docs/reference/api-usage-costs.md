...
Klawty features that can generate provider usage or paid API calls.
...
- `klawty status --usage` and `klawty channels list` show provider **usage windows**
...
Klawty can pick up credentials from:
...
`klawty models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
