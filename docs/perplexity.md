...
Klawty supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Klawty also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `klawty configure --section web`. It stores the key in
`~/.klawty/klawty.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.klawty/.env` (or your service environment). See [Env vars](/help/faq#how-does-klawty-load-environment-variables).
