...
Klawty can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Klawty can send audio; Telegram gets a round voice-note bubble.
...
Klawty defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `klawty.json`.
...
- If `provider` is **unset**, Klawty prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.klawty/settings/tts.json`, override with `KLAWTY_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Klawty retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Klawty registers
