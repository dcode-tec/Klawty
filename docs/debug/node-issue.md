...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Klawty
...
Running Klawty via Node with `tsx` fails at startup with:

```
[klawty] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `klawty status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch klawty.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node klawty.mjs status` works on Node 25.
