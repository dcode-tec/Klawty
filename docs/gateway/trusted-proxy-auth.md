...
read_when:
  - Running Klawty behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Klawty
...
- You run Klawty behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Klawty checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Klawty extracts the user identity from the configured header
...
- Klawty can stay on loopback HTTP behind the proxy.
...
If Klawty itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://klawty.example.com
    to: http://klawty-gateway:2508
    policy:
      - allow:
          or:
            - email:
...
`klawty security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Klawty config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `klawty security audit` and review findings
