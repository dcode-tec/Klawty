...
Klawty supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `KLAWTY_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Klawty runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Klawty validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Klawty QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/klawty"],
...
- Klawty resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `klawty.json` refs)
...
When reload-time activation fails after a healthy state, Klawty enters degraded secrets state.
...
- Strict command paths (for example `klawty memory` remote-memory paths and `klawty qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `klawty status`, `klawty status --all`, `klawty channels status`, `klawty channels resolve`, `klawty security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `klawty secrets reload`.
...
```bash
klawty secrets audit --check
klawty secrets configure
klawty secrets audit --check
```
...
- plaintext values at rest (`klawty.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `klawty.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `klawty secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `klawty.json` plus `auth-profiles.json` for one agent scope
...
- `klawty secrets configure --providers-only`
- `klawty secrets configure --skip-provider-setup`
- `klawty secrets configure --agent <id>`
...
```bash
klawty secrets apply --from /tmp/klawty-secrets-plan.json
klawty secrets apply --from /tmp/klawty-secrets-plan.json --allow-exec
klawty secrets apply --from /tmp/klawty-secrets-plan.json --dry-run
klawty secrets apply --from /tmp/klawty-secrets-plan.json --dry-run --allow-exec
```
...
Klawty intentionally does not write rollback backups containing historical plaintext secret values.
