...
read_when:
  - Setting up Klawty for the first time
...
Klawty reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.klawty/klawty.json`.

If the file is missing, Klawty uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `klawty onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    klawty onboard       # full onboarding flow
    klawty configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    klawty config get agents.defaults.workspace
    klawty config set agents.defaults.heartbeat.every "2h"
    klawty config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  <Tab title="Control UI">
    Open [http://127.0.0.1:2508](http://127.0.0.1:2508) and use the **Config** tab.
    The Control UI renders a form from the config schema, with a **Raw JSON** editor as an escape hatch.
  </Tab>
  <Tab title="Direct edit">
    Edit `~/.klawty/klawty.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Klawty only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`klawty doctor`, `klawty logs`, `klawty health`, `klawty status`)
- Run `klawty doctor` to see exact issues
- Run `klawty doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `klawty.json`.
    ...
    ```bash
    klawty config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `KLAWTY_APNS_RELAY_BASE_URL` and `KLAWTY_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `KLAWTY_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.klawty/klawty.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `klawty config set` for single keys.
    </Warning>
    ...
    ```bash
    klawty gateway call config.get --params '{}'  # capture payload.hash
    klawty gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.klawty/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    klawty gateway call config.patch --params '{
...
Klawty reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.klawty/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Klawty runs your login shell and imports only the missing keys:
...
Env var equivalent: `KLAWTY_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${KLAWTY_GATEWAY_TOKEN}" } },
