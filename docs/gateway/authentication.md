...
Klawty supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `klawty gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.klawty/.env` so the daemon can read it:
...
API keys for daemon use: `klawty onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.klawty/.env`, systemd/launchd).
...
```bash
klawty models auth paste-token --provider anthropic
klawty models auth paste-token --provider openrouter
```
...
- Priority order:
  - `KLAWTY_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Klawty retries with the next key only for rate-limit errors (for example
...
Run `klawty models status` to confirm which profile is expiring. If the profile
