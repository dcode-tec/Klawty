...
- `/usage cost` in chats: local cost summary aggregated from Klawty session logs.
- CLI: `klawty status --usage` prints a full per-provider breakdown.
- CLI: `klawty channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
