...
read_when:
  - You want to understand how Klawty assembles model context
...
A **context engine** controls how Klawty builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Klawty ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.klawty/klawty.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Klawty plugin. Install
first, then select the engine in the slot:

```bash
# Install from npm
klawty plugins install @martian-engineering/lossless-claw

# Or install from a local path (for development)
klawty plugins install -l ./my-context-engine
```
...
Every time Klawty runs a model prompt, the context engine participates at
...
Klawty currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Klawty
...
The built-in `legacy` engine preserves Klawty's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Klawty uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Klawty disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Klawty automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `klawty/plugin-sdk/core` to use
  Klawty's built-in compaction behavior.
...
Klawty resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Klawty's built-in summarization. Plugin engines can implement
...
- Use `klawty doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Klawty
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `klawty plugins install -l ./my-engine` to link a
