---
title: "Memory"
summary: "How Klawty memory works (workspace files + automatic memory flush)"
read_when:
...
Klawty memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, Klawty only loads `MEMORY.md`.
  - Lowercase `memory.md` is only used as a fallback when `MEMORY.md` is absent.
  - **Only load in the main, private session** (never in group contexts).

These files live under the workspace (`agents.defaults.workspace`, default
`~/.klawty/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

Klawty exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, Klawty triggers a **silent,
...
Klawty can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, Klawty auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. Klawty
...
BM25 + vectors + reranking. Markdown stays the source of truth; Klawty shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.klawty/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.klawty/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, Klawty retries with `qmd query`. If QMD fails or the binary is
  missing, Klawty automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- Klawty does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - Klawty sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index Klawty
    uses), run a one-off query with the agent’s XDG dirs.

    Klawty’s QMD state lives under your **state dir** (defaults to `~/.klawty`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    Klawty uses:

    ```bash
    # Pick the same state dir Klawty uses
    STATE_DIR="${KLAWTY_STATE_DIR:-$HOME/.klawty}"
...
- When `scope` denies a search, Klawty logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, Klawty exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.klawty/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- If `memorySearch.multimodal.enabled = true`, Klawty also indexes supported image/audio files under `extraPaths` only. Default memory roots (`MEMORY.md`, `memory.md`, `memory/**/*.md`) stay Markdown-only.
- Symlinks are ignored (files or directories).

### Multimodal memory files (Gemini image + audio)

Klawty can index image and audio files from `memorySearch.extraPaths` when using Gemini embedding 2:
...
> Klawty will automatically reindex when it detects a model or dimension change.
...
- Index storage: per-agent SQLite at `~/.klawty/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, Klawty automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, Klawty combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Klawty falls back to vector-only search.
...
Klawty can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.klawty/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, Klawty stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Klawty logs the
