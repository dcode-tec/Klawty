---
summary: "CLI reference for `klawty plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
klawty plugins list
klawty plugins install <path-or-spec>
klawty plugins inspect <id>
klawty plugins enable <id>
klawty plugins disable <id>
klawty plugins uninstall <id>
klawty plugins doctor
klawty plugins update <id>
klawty plugins update --all
klawty plugins marketplace list <marketplace>
```

Bundled plugins ship with Klawty but start disabled. Use `plugins enable` to
activate them.

Native Klawty plugins must ship `klawty.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: klawty` or `Format: bundle`. Verbose list/info
...
```bash
klawty plugins install <path-or-spec>
klawty plugins install <npm-spec> --pin
klawty plugins install <plugin>@<marketplace>
klawty plugins install <plugin> --marketplace <marketplace>
```
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Klawty
...
```bash
klawty plugins install <plugin-name> --marketplace <marketplace-name>
klawty plugins install <plugin-name> --marketplace <owner/repo>
klawty plugins install <plugin-name> --marketplace ./my-marketplace
```
...
For local paths and archives, Klawty auto-detects:

- native Klawty plugins (`klawty.plugin.json`)
...
```bash
klawty plugins install -l ./my-plugin
```
...
```bash
klawty plugins uninstall <id>
klawty plugins uninstall <id> --dry-run
klawty plugins uninstall <id> --keep-files
```
...
state dir extensions root (`$KLAWTY_STATE_DIR/extensions/<id>`). Use
...
klawty plugins update <id>
klawty plugins update --all
klawty plugins update <id> --dry-run
```
...
```bash
klawty plugins inspect <id>
klawty plugins inspect <id> --json
```
