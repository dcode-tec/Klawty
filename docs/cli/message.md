---
summary: "CLI reference for `klawty message` (send + channel actions)"
read_when:
...
```
klawty message <subcommand> [flags]
```
...
- On cache miss, Klawty will attempt a live directory lookup when the provider supports it.
...
- `klawty message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Klawty does not force a `default` account SecretRef scope
...
```
klawty message send --channel discord \
...
```
klawty message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
klawty message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
klawty message poll --channel msteams \
...
```
klawty message react --channel slack \
...
```
klawty message react --channel signal \
...
```
klawty message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
klawty message send --channel telegram --target @mychat \
