---
summary: "CLI reference for `klawty onboard` (interactive onboarding)"
read_when:
...
```bash
klawty onboard
klawty onboard --flow quickstart
klawty onboard --flow manual
klawty onboard --mode remote --remote-url wss://gateway-host:2508
```

For plaintext private-network `ws://` targets (trusted networks only), set
`KLAWTY_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `klawty gateway run`.
...
- Fastest first chat: `klawty dashboard` (Control UI, no channel setup).
