---
summary: "CLI reference for `klawty memory` (status/index/search)"
read_when:
...
```bash
klawty memory status
klawty memory status --deep
klawty memory index --force
klawty memory search "meeting notes"
klawty memory search --query "deployment" --max-results 20
klawty memory status --json
klawty memory status --deep --index
klawty memory status --deep --index --verbose
klawty memory status --agent main
klawty memory index --agent main --verbose
```
