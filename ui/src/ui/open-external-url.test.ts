...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://klawty.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://klawty.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://klawty.ai/abc-123", baseHref)).toBe(
      "blob:https://klawty.ai/abc-123",
    );
  });
