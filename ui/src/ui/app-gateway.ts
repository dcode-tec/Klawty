...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void subscribeSessions(host as unknown as KlawtyApp);
      void loadAssistantIdentity(host as unknown as KlawtyApp);
      void loadAgents(host as unknown as KlawtyApp);
      void loadHealthState(host as unknown as KlawtyApp);
      void loadNodes(host as unknown as KlawtyApp, { quiet: true });
      void loadDevices(host as unknown as KlawtyApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (runId && host.refreshSessionsAfterChat.has(runId)) {
    ...
    if (state === "final") {
      void loadSessions(host as unknown as KlawtyApp, {
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as KlawtyApp);
    return true;
  }
  return false;
}
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as KlawtyApp, payload);
