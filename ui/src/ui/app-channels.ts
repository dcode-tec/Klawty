...
export async function handleWhatsAppStart(host: KlawtyApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: KlawtyApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: KlawtyApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: KlawtyApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: KlawtyApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: KlawtyApp): string {
...
function resolveGatewayHttpAuthHeader(host: KlawtyApp): string | null {
...
function buildGatewayHttpHeaders(host: KlawtyApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: KlawtyApp) {
...
export function handleNostrProfileToggleAdvanced(host: KlawtyApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: KlawtyApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: KlawtyApp) {
  const state = host.nostrProfileFormState;
