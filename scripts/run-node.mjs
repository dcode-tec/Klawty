...
const extensionRestartMetadataFiles = new Set(["klawty.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.KLAWTY_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.KLAWTY_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[klawty] ${message}\n`);
};

const runKlawty = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["klawty.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
