...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("klawty/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("klawty/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced klawty/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced klawty/plugin-sdk/<subpath> must exist in the public package exports.",
  );
