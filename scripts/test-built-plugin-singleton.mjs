...
const { clearPluginCommands, getPluginCommandSpecs, loadKlawtyPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadKlawtyPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "klawty-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@klawty/build-smoke-plugin",
      type: "module",
      klawty: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'klawty/plugin-sdk';",
...
const registry = loadKlawtyPlugins({
  ...
  env: {
    ...process.env,
    KLAWTY_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
    KLAWTY_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
  },
  config: {
    plugins: {
      enabled: true,
