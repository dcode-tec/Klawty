---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Klawty deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Klawty cron scheduling for periodic checks, or version status checks on a machine running Klawty (laptop, workstation, Pi, VPS).
---

# Klawty Host Hardening

## Overview

Assess and harden the host running Klawty, then align it to a user-defined risk tolerance without breaking access. Use Klawty security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Klawty changes the host firewall, SSH, or OS updates; it does not.
...
5. Klawty gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Klawty can access sensitive data.
...
### 2) Run Klawty security audits (read-only)

As part of the default read-only checks, run `klawty security audit --deep`. Only offer alternatives if the user requests them:

1. `klawty security audit` (faster, non-probing)
2. `klawty security audit --json` (structured output)

Offer to apply Klawty safe defaults (numbered):

1. `klawty security audit --fix`

Be explicit that `--fix` only tightens Klawty defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Klawty version/update status (read-only)

As part of the default read-only checks, run `klawty update status`.
...
- Credential hygiene notes (location of Klawty creds, prefer disk encryption)
...
After Klawty install or first hardening pass, run at least one baseline audit and version check:

- `klawty security audit`
- `klawty security audit --deep`
- `klawty update status`

Ongoing monitoring is recommended. Use the Klawty cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `klawty cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `klawty update status`
...
Before creating, `klawty cron list` and match on exact `name`. If found, `klawty cron edit <id> ...`.
If not found, `klawty cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `klawty update status` (preferred for source checkouts and channels)
2. `npm view klawty version` (published npm version)

## Klawty command accuracy

Use only supported commands and flags:

- `klawty security audit [--deep] [--fix] [--json]`
- `klawty status` / `klawty status --deep`
- `klawty health --json`
- `klawty update status`
- `klawty cron add|list|runs|run`

Do not invent CLI flags or imply Klawty enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Klawty compaction:
