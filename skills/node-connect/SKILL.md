...
Goal: find the one real route from node -> gateway, verify Klawty is advertising that route, then fix pairing/auth.
...
- whether `klawty devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `klawty qr --json`. It uses the same setup-code payload Android scans.

```bash
klawty config get gateway.mode
klawty config get gateway.bind
klawty config get gateway.tailscale.mode
klawty config get gateway.remote.url
klawty config get gateway.auth.mode
klawty config get gateway.auth.allowTailscale
klawty config get plugins.entries.device-pair.config.publicUrl
klawty qr --json
klawty devices list
klawty nodes status
```

If this Klawty instance is pointed at a remote gateway, also run:

```bash
klawty qr --remote --json
```
...
If `klawty qr --json` says `Gateway is only bound to loopback`:
...
- `klawty devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run klawty qr again, rescan, then approve the pending device pairing.`
